...
`PASS_LIVE_PRODUCTION_OWNER_SHARE_SESSION_REVOCATION_RESCAN_PENDING`

Live production commit `809f1feb` adds the missing operator revocation path for a saved Share session without changing the existing schema. The authenticated manager action uses `DELETE /api/workpacks/[id]/share-sessions?sessionId=[uuid]`, reloads the owned workpack context, and scopes the update by session, workpack, organization, and site before persisting `status=revoked`. The returned `updated_at` value is the durable audit timestamp.
...
- Diff check: PASS.
- Production build marker: `809f1feb`, branch `master`, environment `production`.
- Unauthenticated live DELETE probe: HTTP 401, configured true, `revokedSessionId=null`.
