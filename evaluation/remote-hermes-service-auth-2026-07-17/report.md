...
- Timeout race remediation base: `dc834ac710f1d5d2d995c54d05a71f6a0d5661fd`
...
- Marks timeout before settling its rejection and aborting the consumer, then rechecks timeout state after the race.
- Rejects an abort listener that synchronously resolves a direct consumer Promise to `true` under fake and real timers.
...
- Timeout race RED: 31 tests, 1 failed because an abort listener's direct Promise resolved `true` before timeout rejection settled.
- Timeout race GREEN: 32 tests passed under fake and real timers with no unhandled rejection.
