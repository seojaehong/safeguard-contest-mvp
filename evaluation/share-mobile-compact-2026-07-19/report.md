...
| Build marker | Production `/api/build-info` returned `eb230c865f7b7ff3ce4e235c45ed3e9d05b9d65a`, branch `master`, environment `production` |
...
| --- | ---: | ---: | --- |
| Mobile share height | 2296px / 2.72x original baseline | 1926px / 2.28x | Improved, still long |
| Mobile share panel | 1893px local regression baseline | 1521px | Improved, still long |
| Mobile share form | 746px | 726px | Slightly improved |
...
- `node evaluation/workspace-ux-current-2026-07-19/measure_workspace_ux.cjs`: PASS against `https://www.safeclaw.kr`, production build `eb230c865f7b7ff3ce4e235c45ed3e9d05b9d65a`

## Remaining Gate

- GitHub Actions run `29690411928` is still running the full test gate for this head.
- Mobile share is no longer clipped and preserves the full Vietnamese preview contract, but it is still longer than a single viewport; the later Share v2 IA pass should keep only target, channel, preview, and one send CTA on the default surface.
