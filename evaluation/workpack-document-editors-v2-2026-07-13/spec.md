...
- `remediationParent`: `75b522e8ee49f86482300a5bb3e3ae4ff0dc09ac`
...
| --- | --- | --- | --- | --- | --- | --- |
| "$" | "object" | 19 | 4544 | 8 | "sha256:dbbf25a60f96502f961a8e48111a780b8b0f4abc0fe0ebbb41afff7dc1e1bd34" | "&lt;bound by complete top-level domain rows&gt;" |
...
| "schemaVersion" | "string" | 0 | 1 | 0 | "sha256:b9ee16917afb7bb24688826eabed6df6bcb6974115222eb318e4d565cfaa291e" | "2.7.0" |
...
- **HP-002 Provenance:** The true source base is d3ad86530bc786d8024206cc5b7c7db60c055278, the bound integration target is f98ae7d16746dfe9fedbeea892e5af7ebb56f9a5, and this candidate descends from remediation parent 75b522e8ee49f86482300a5bb3e3ae4ff0dc09ac.
...
- **HP-006 Trust:** The current repository cannot authenticate a Codex/user approval event. Approval/provider/CI/DOM receipt requirements remain future and normative only; implementation mode accepts nothing and always returns IMPLEMENTATION_BLOCKED_PENDING_USER_DB_APPROVAL.
...
This repository cannot authenticate a Codex/user approval event today. A repository-authored manifest, JSON boolean, hash, commit message, validator output, or agent statement is therefore not authoritative approval. Approval-event identity, provider/CI receipts, artifact binding, and DOM census/measurement formats remain future normative requirements only. A separate post-approval verifier may be designed and reviewed only after the user explicitly approves the DB migration/transactional RPC authority outside this repository.

Every timestamp that this document contract treats as a timestamp uses strict RFC3339 calendar/time/offset validation. The validator does not use permissive `Date.parse`, and implementation mode accepts no receipt timestamp because it accepts no receipt at all.
...
Every deliberate RED case must exit nonzero twice. Spec-review mutations cover Markdown-only prose, JSON-only model and `primaryAction`, an unknown domain, missing/empty domains, empty document fields/actions, broken photo relationships, empty conflict heads/refs, an unblocked wave, and candidate/source/target/evidence-parent identity. Implementation attacks prove empty, forged, and complete-looking evidence all hit the same unconditional block:
