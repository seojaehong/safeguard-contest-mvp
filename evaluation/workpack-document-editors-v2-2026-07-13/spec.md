...
- `remediationParent`: `ee8e314489f9326d68080cf7bca3cee86226f9de`
...
- Raw photo bytes, File/Blob values, object URLs, local previews, and unapproved EXIF remain site-memory/session only. They never enter editorV2, approval evidence, command logs, browser sidecars, or the review event.
...
| `BROWSER-025` | 200% independent containment | raw sidecars contain no pass/result; validator recomputation finds zero cross-parent overlap, fixed/sticky escape, ancestor clipping, inner transform/zoom, document overflow, nested editor/textarea scroll, or late mobile editor start |

### ZOOM-001 Actual 200 Percent Contract

The implementation matrix is Chromium, Firefox, and WebKit x `desktop1440` and `mobile390` x Day/Night x all 12 document keys: 144 cases. Each fresh context starts with `deviceScaleFactor=1`, `devicePixelRatio=1`, `visualViewport.scale=1`, `data-safeclaw-text-policy=baseline`, root `--safeclaw-type-scale=1`, and `text-size-adjust=100%`.

The test harness captures a raw baseline sidecar, then in one evaluate transaction sets `data-safeclaw-text-policy=double`, root `--safeclaw-type-scale=2`, and harness-owned `applicationCount=1`. Typography tokens consume that root value. Browser-native zoom, CSS `zoom`, transforms, screenshot scaling, device-scale changes, nested multipliers, and a second application are forbidden.

Each immutable sidecar records source SHA, build ID, fixture identity, viewport policy values, baseline/scaled page metrics, and every measured node's parent, role, visibility, DOMRect, client/scroll dimensions, line count, and computed font/line/overflow/position/transform/zoom/text-size/max-height values. It contains no `pass`, `result`, claimed metric, or free-form output hash.

The validator loads and hashes all 144 sidecars from the successful browser receipt, then independently computes text ratios and reflow, fixed/sticky viewport containment, all cross-parent priority intersections, clipping through every overflow ancestor, inner transform/zoom, document overflow, nested scroll, textarea hidden-scroll, 44px targets, 8px stack gap, and mobile editor root/heading y budgets. Fourteen explicit negative fixtures cover pass flags, cumulative scale, transform/zoom, overlap, offscreen fixed/sticky, x/y clipping, nested/textarea scroll, late mobile start, bad ratio/reflow, and pixel/viewport scaling. Spec-review proves those fixtures fail but prints no browser behavior PASS; implementation mode may print one only after all raw evidence recomputes cleanly.
...
- **HP-001 Hold:** Status is HOLD_PENDING_FRESH_REVIEW and the entire implementation program, including Wave 0, remains BLOCKED_PENDING_USER_DB_APPROVAL until a fresh independent spec PASS and an externally evidenced explicit user DB authority approval both exist.
- **HP-002 Provenance:** The true source base is d3ad86530bc786d8024206cc5b7c7db60c055278, the bound integration target is f98ae7d16746dfe9fedbeea892e5af7ebb56f9a5, and this candidate descends from remediation parent ee8e314489f9326d68080cf7bca3cee86226f9de.
...
- **HP-006 Trust:** Implementation validation accepts neither approval booleans nor free-form command hashes. It requires separate external approval events, the exact migration/RPC proposal blob, CI execution receipts, actual logs/artifacts, and independently recomputed raw browser sidecars.
- **HP-007 Typography:** The 200 percent harness applies one root typography-token policy at devicePixelRatio 1 and visualViewport scale 1, then recomputes ratios, reflow, fixed/sticky containment, cross-parent overlap, clipping, transforms, zoom, nested scroll, and mobile editor start without trusting pass flags.
...
- `implementation` requires an immutable spec commit; explicit nonempty unequal base/head; a separate external approval commit and manifest; a distinct implementation evidence commit; a nonempty owned-file range; and receipt-bound logs/artifacts. It does not accept inline approval/pass booleans, free-form command strings, unhashed output, or arbitrary digest assertions.

The external approval manifest binds two raw provider events: `INDEPENDENT_SPEC_PASS` from a GitHub review event and `USER_DB_AUTHORITY_APPROVAL` from the explicit Codex user-approval event. Each binds immutable event identity, repository, actor/login, timestamp, decision, spec SHA, target SHA, migration/RPC proposal digest, root operation ID, and its actual hashed raw-event blob. The same commit contains the exact proposal blob; validator recomputation, not a supplied 64-hex value, establishes its digest.

Each command uses a `safeclaw-command-execution-receipt/v1` blob with source SHA, build ID, command ID, argv array, cwd, provider run/job identity, start/end time, loaded exit code, and hashed stdout/stderr/artifact blobs. For `ZOOM-001`, all 144 raw sidecars are browser-receipt artifacts and the validator recomputes the metrics described above. A manifest cannot turn a failure into PASS.
...
Every deliberate RED case must exit nonzero twice. The spec-review cases mutate Markdown prose, structural JSON including model and `primaryAction`, unknown domains, target/source refs, approval/command trust, and every browser negative class. `implementation-empty` proves empty refs fail before any future manifest can claim execution:

```powershell
1..2 | ForEach-Object {
  foreach ($case in @(
    'normative-parity','md-prose','json-model','json-document-primary-action','json-unknown-domain',
    'source-shape','target-ref','spec-ref','approval-boolean','command-unbound-hash',
    'browser-pass-flag','browser-cumulative-scale','browser-inner-transform','browser-inner-zoom',
    'browser-cross-parent-overlap','browser-fixed-offscreen','browser-sticky-cover',
    'browser-horizontal-clip','browser-vertical-clip','browser-nested-scroll','browser-textarea-scroll',
    'browser-mobile-late','browser-ratio-reflow','browser-pixel-viewport-scale'
  )) {
...
```powershell
node $validator implementation --spec <immutable-spec-SHA> --approval-evidence <external-approval-evidence-SHA> --approval-manifest <external-approval-manifest-path> --evidence <implementation-evidence-SHA> --manifest <implementation-manifest-path> --base <approved-target-SHA> --head <non-empty-head-SHA>
```
