...
| SRC-19 | `components/WorkpackEditor.tsx -> app/api/export/xlsx/route.ts` | `downloadXlsx -> POST` | At target `f45bba17bcce0d8ebb2690f82d014dbe42ae8191`, the editor posts single or one of five structured modes to `/api/export/xlsx`; route integration belongs only to Wave 5. |
...
Actual target `f45bba17bcce0d8ebb2690f82d014dbe42ae8191` seams, all owned only by future Wave 5, are:
...
`spec.json` is the canonical source. The structural block below enumerates every top-level domain without an allowlist; each domain row contains its complete nested value and the root row independently binds the whole contract. Every nested object and union member is recursively closed by an independently fixed schema-shape fingerprint except the two documented string maps. A second generated block binds the concise human requirements, while normalized prose outside both blocks is also hash-bound from JSON.
...
| --- | --- | --- | --- | --- | --- | --- |
| "$" | "object" | 19 | 4617 | 8 | "sha256:0e721c5c0970cedc4154e07b7b81b83a18c7ff7686a965affbb2b9a33df5b4ed" | "&lt;bound by complete top-level domain rows&gt;" |
...
- **HP-002 Provenance:** The target-ready candidate has source base, integration target, and direct parent f45bba17bcce0d8ebb2690f82d014dbe42ae8191; its evidence-only commit must directly parent the candidate.
...
- **HP-006 Trust:** The current repository cannot authenticate a Codex/user approval event. Claims are closed zero-execution declarations; every nested schema object and union member rejects unknown keys except two documented string maps; future approval/provider/CI/DOM requirements accept no result fields; evidence time is strict RFC3339 and no more than 300 seconds ahead of guarded validation time; and implementation mode always returns IMPLEMENTATION_BLOCKED_PENDING_USER_DB_APPROVAL.
...
```text
node evaluation/workpack-document-editors-v2-2026-07-13/validate-contract.mjs spec-review --evidence <FULL_EVIDENCE_SHA> --manifest evaluation/workpack-document-editors-v2-2026-07-13/review-evidence.json --candidate <FULL_CANDIDATE_SHA> --source-base f45bba17bcce0d8ebb2690f82d014dbe42ae8191 --target f45bba17bcce0d8ebb2690f82d014dbe42ae8191 --validation-time <STRICT_RFC3339_CURRENT_TIME>
```
...
$sourceBase = 'f45bba17bcce0d8ebb2690f82d014dbe42ae8191'
$target = 'f45bba17bcce0d8ebb2690f82d014dbe42ae8191'
...
Every deliberate RED case must exit nonzero twice. Spec-review mutations cover Markdown-only prose, JSON-only model and `primaryAction`, an unknown domain, missing/empty domains, empty document fields/actions, broken photo relationships, empty conflict heads/refs, an unblocked wave, forged and complete-looking spec/evidence claims, synthetic geometry results, 2099 evidence, arbitrary wave commands, and candidate/source/target/evidence-parent identity. The separate unknown-key matrix clones all 255 closed nested object instances and must reject every injected key through the recursive closure gate; it explicitly covers six raw provenance variants, four photo relationships, fields, actions, states, routes, commands, browser families, and complete-looking claim objects. Implementation attacks prove empty, forged, and complete-looking evidence all hit the same unconditional block:

```powershell
1..2 | ForEach-Object {
  node $validator attack-check --deliberate unknown-key-matrix
  if ($LASTEXITCODE -ne 0) { throw 'unknown-key matrix failed' }
}
...
1..2 | ForEach-Object {
  foreach ($case in @(
    ...
    'forged-spec-claims','forged-evidence-claims','complete-looking-spec-claims','complete-looking-evidence-claims',
