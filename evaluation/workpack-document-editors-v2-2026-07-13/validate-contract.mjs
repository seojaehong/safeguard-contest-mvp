...
const SOURCE_CANDIDATE_SHA = "c2ed91db690b9db508580099590130e0cf057209";
const SOURCE_CANDIDATE_BRANCH = "feat/workpack-document-editors-v2-target-ready-v3";
...
const BRANCH = "feat/workpack-document-editors-v2-target-ready-v4";
...
const EXPECTED_SHAPE_SHA256 = "sha256:fa7a82debe7866036779c194adb3164a5eb63d94c6841755d06b5b7cdf1e5226";
...
const V3_CORRECTION_FAILURES = [
  "missing owning-root textScalingContract",
  "positive mobile rows used selected width plus one",
  "missing synthetic leaf-scaling rejection attack",
  "missing nonselected mobile viewport rejection attack"
];
...
const OBJECT_KEYS = new Map([
  ...
  ["v3_correction_red_baseline", ["kind", "referenceBranch", "referenceCommit", "baselineAuthoringExit", "correctionGateExit", "failures", "closedObjects", "positiveMobileWrongWidthRows", "baselineAuthoringRuntimeMs", "correctionGateRuntimeMs", "browserExecutions"]]
]);
...
function validateReviewScope(scope) {
  ...
  if (
    ...
    throw new Error("IDENTITY: read-only v3 source candidate provenance differs");
  }
...
function validateValidationContract(contract) {
  ...
  if (
    ...
    contract.requiredRuns !== "Canonical spec-review twice, dynamic unknown-key matrix twice, and all 26 deliberate negative attacks twice."
  ) {
    throw new Error("VALIDATION_CONTRACT: all 24 v3 attacks plus the two browser correction attacks are required in order");
  }
...
function validateContract(spec, validationTime, options = {}) {
  ...
  if (spec.schemaVersion !== "4.0.0") throw new Error("SCHEMA_VERSION: expected 4.0.0");
...
function buildSyntheticEvidenceManifest(spec, validationTime) {
  ...
  return {
    ...
    v3CorrectionRedBaseline: {
      kind: "v3_correction_red_baseline",
      referenceBranch: SOURCE_CANDIDATE_BRANCH,
      referenceCommit: SOURCE_CANDIDATE_SHA,
      baselineAuthoringExit: 0,
      correctionGateExit: 1,
      failures: V3_CORRECTION_FAILURES,
      closedObjects: 328,
      positiveMobileWrongWidthRows: 3,
      baselineAuthoringRuntimeMs: 769,
      correctionGateRuntimeMs: 872,
      browserExecutions: 0
    }
  };
}
...
function validateCandidateIdentity(root, candidate, sourceBase, target, spec) {
  if (candidate === SOURCE_CANDIDATE_SHA || candidate === REJECTED_CANDIDATE_SHA || candidate === REJECTED_EVIDENCE_SHA) {
    throw new Error("IDENTITY: source v3 or rejected v2 commit cannot be the v4 candidate");
  }
  ...
  if (commitParent(root, SOURCE_CANDIDATE_SHA) !== BASE_SHA || isAncestor(root, SOURCE_CANDIDATE_SHA, candidate)) {
    throw new Error("IDENTITY: v3 must remain a sibling read-only content source, never candidate ancestry");
  }
...
function validateEvidenceManifestShape(manifest, spec, validationTime) {
  collectClosedObjectsAndRequireExactKeys(manifest, "$manifest");
  if (manifest.schemaVersion !== "4.0.0" || manifest.branch !== BRANCH) {
  ...
  const correction = manifest.v3CorrectionRedBaseline;
  if (
    correction.referenceBranch !== SOURCE_CANDIDATE_BRANCH ||
    correction.referenceCommit !== SOURCE_CANDIDATE_SHA ||
    correction.baselineAuthoringExit !== 0 ||
    correction.correctionGateExit !== 1 ||
    canonicalJson(correction.failures) !== canonicalJson(V3_CORRECTION_FAILURES) ||
    correction.closedObjects !== 328 ||
    correction.positiveMobileWrongWidthRows !== 3 ||
    correction.browserExecutions !== 0
  ) {
    throw new Error("VALIDATION_EVIDENCE: v3 browser text-scaling correction RED baseline differs");
  }
  requireInteger(correction.baselineAuthoringRuntimeMs, "v3 correction baseline authoring runtime", 0);
  requireInteger(correction.correctionGateRuntimeMs, "v3 correction gate runtime", 0);
}
