...
const SPEC_REVIEW_COMMAND_TOKENS = [
  ...
  "<FULL_CANDIDATE_SHA>",
  "--source-base",
  "f98ae7d16746dfe9fedbeea892e5af7ebb56f9a5",
  "--target",
  "f98ae7d16746dfe9fedbeea892e5af7ebb56f9a5",
  "--validation-time",
...
function validateValidationDomain(validation) {
  requireExactKeys(
    validation,
    ["currentGate", "semanticParity", "modes", "candidateArtifacts", "evidenceManifest", "targetBlobPaths", "currentClaims", "notCurrentlyProved", "redCases", "validator", "implementationProgramGate", "timestampPolicy", "commandContracts", "futurePostApprovalVerifierRequirements"],
    "validation"
  );
...
function validateContractSchema(spec, validationTimeMilliseconds = Date.now()) {
  ...
  if (spec.meta.sourceBase !== "f98ae7d16746dfe9fedbeea892e5af7ebb56f9a5") throw new Error("sourceBase differs from reviewed source");
  ...
  if (spec.meta.remediationParent !== "f98ae7d16746dfe9fedbeea892e5af7ebb56f9a5") throw new Error("remediationParent differs from the authoritative target");
  if (spec.meta.branch !== "feat/workpack-document-editors-v2-target-ready") throw new Error("meta.branch differs from the target-ready branch");
}
...
function runAttackCheck(args) {
  ...
  if (args.deliberate === "forged-evidence-claims" || args.deliberate === "future-evidence-time") {
...
function main() {
  ...
  throw new Error("Mode must be render-normative, render-human, render-prose-digest, authoring-check, attack-check, spec-review, or implementation");
}
