...
- Reports remediation commit `b42bc0d` follows rejected candidate `a935fdb` and implements fail-closed server-error handling plus explicit local switching. It remains outside backend `cab4a19` pending a fresh independent verdict and integration checks.
- PDF selective port commit `66a05fe` ports authoritative frontend snapshot `b61929f` onto `3900892` with a regenerated tracked lockfile, excluding `WorkpackEditor` and `globals.css`. Local evidence covers Korean extraction, a nonblank rendered first page, representative output below 1 MiB, controlled missing/invalid-font 500, literal NFT assets, typecheck, and production build. It remains outside backend `cab4a19` pending independent review and combined integration checks.
