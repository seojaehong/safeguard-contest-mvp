...
import { PROVIDER_DISPATCH_IDEMPOTENCY_SUPPORTED } from "@/lib/server/workflow-dispatch-capability-policy";
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    providerDispatchIdempotencySupported: PROVIDER_DISPATCH_IDEMPOTENCY_SUPPORTED,
    message: "검증되지 않은 클라이언트 provider 결과는 저장하지 않습니다. 서버 영수증 persistence 승인 후 authoritative dispatch 경로에서만 기록합니다.",
  }, { status: 409 });
}
