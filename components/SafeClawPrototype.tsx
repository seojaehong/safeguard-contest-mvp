...
const screens: PrototypeScreen[] = [
  ...
  { group: "제품", id: "home", label: "홈 대시보드", num: "04", title: "오늘 처리할 현장을 먼저 보여줍니다.", subtitle: "다현장 상태, API 연결, 미완료 교육, 전파 실패를 한 화면에서 확인합니다.", primary: "현장 현황", status: "planned", mappedTo: "/home · 운영 대시보드 후보", scope: ["다현장 카드", "미완료 교육", "전파 실패 요약"], metrics: [["현장", "다현장"], ["알림", "운영"]]},
  ...
  { group: "제품", id: "workspace-b", label: "워크스페이스 B", num: "06", title: "입력과 결과를 단일 컬럼으로 압축합니다.", subtitle: "태블릿과 좁은 화면에서 쓰기 쉬운 작업공간 변형입니다.", primary: "단일 패널", status: "planned", mappedTo: "/workspace · 반응형 리패스 후보", scope: ["태블릿 레이아웃", "입력 우선", "결과 접힘"], metrics: [["초점", "입력"], ["화면", "태블릿"]]},
  { group: "제품", id: "docs", label: "문서 편집", num: "07", title: "문서별 편집과 보완 제안을 나란히 둡니다.", subtitle: "위험성평가, 작업계획서, TBM, 안전교육 기록을 제출 서식 기준으로 편집합니다.", primary: "3-pane", status: "live", mappedTo: "/documents · WorkpackEditor", scope: ["문서별 탭", "보완 문구 생성", "XLS/HWPX/PDF 출력"], metrics: [["정식", "XLS/HWPX/PDF"], ["보완", "삽입"]]},
  { group: "제품", id: "evidence", label: "근거 라이브러리", num: "08", title: "법령, KOSHA, 재해사례를 문서 문장에 연결합니다.", subtitle: "인용할 근거와 현재 문서 반영 위치를 함께 보여주는 화면입니다.", primary: "근거 매핑", status: "partial", mappedTo: "/evidence · CitationList + KOSHA", scope: ["법령/해석례/판례", "KOSHA 자료", "문서 반영 근거"], metrics: [["법령", "조문"], ["자료", "KOSHA"]]},
  { group: "제품", id: "workers", label: "작업자 · 교육", num: "09", title: "작업자, 언어, 교육 상태를 한곳에서 봅니다.", subtitle: "연락처, 국적, 주 사용 언어, 신규 여부, 교육 확인 상태를 최소 정보로 관리합니다.", primary: "교육 확인", status: "partial", mappedTo: "/workers · Worker/Education shell", scope: ["작업자 대상", "교육 상태", "언어별 안내"], metrics: [["작업자", "명단"], ["언어", "자동"]]},
  { group: "제품", id: "dispatch", label: "현장 전파", num: "10", title: "메일과 문자 전송 결과를 기록합니다.", subtitle: "카카오와 밴드는 준비 중으로 잠그고, 제출 기준은 메일과 문자로 고정합니다.", primary: "실발송 로그", status: "live", mappedTo: "/dispatch · WorkflowSharePanel", scope: ["n8n dispatch", "메일/SMS", "전파 로그"], metrics: [["메일", "연결"], ["문자", "연결"]]},
  { group: "필드", id: "mobile", label: "작업자 모바일", num: "11", title: "작업자는 핵심 공지만 확인합니다.", subtitle: "작업자용 화면은 길게 설명하지 않고 위험, 조치, 확인만 보여줍니다.", primary: "모바일 확인", status: "planned", mappedTo: "/worker · 작업자 모바일 후보", scope: ["쉬운 한국어", "외국어 전송본", "확인 버튼"], metrics: [["문장", "짧게"], ["언어", "선택"]]},
  { group: "필드", id: "tbm", label: "TBM 풀스크린", num: "12", title: "현장 조회용 TBM 화면입니다.", subtitle: "작업 전 회의에서 크게 띄워 읽는 위험요인, 질문, 확인 항목 중심 화면입니다.", primary: "회의 모드", status: "planned", mappedTo: "/tbm · 풀스크린 TBM 후보", scope: ["TBM 브리핑", "확인 질문", "출석/서명"], metrics: [["질문", "3개"], ["확인", "서명"]]},
  { group: "시스템", id: "archive", label: "이력 · 아카이브", num: "13", title: "과거 문서팩과 전파 이력을 검색합니다.", subtitle: "현장, 날짜, 공정, 작업자, 위험유형 기준으로 이전 기록을 찾습니다.", primary: "검색 이력", status: "partial", mappedTo: "/archive · workpacks/dispatch_logs", scope: ["문서팩 저장", "전파 이력", "검색 UI 후속"], metrics: [["문서팩", "저장"], ["로그", "전파"]]},
  ...
  { group: "시스템", id: "api", label: "API 연결", num: "15", title: "공공 API 연결 상태를 운영자가 확인합니다.", subtitle: "기상청, Law.go, KOSHA, Work24, Gemini, n8n 연결 상태를 점검합니다.", primary: "연결 점검", status: "live", mappedTo: "/ops/api · API smoke reports", scope: ["dryrun 리포트", "API smoke", "제출 준비 점검"], metrics: [["상태", "채널별"], ["로그", "저장"]]},
  { group: "시스템", id: "settings", label: "설정", num: "16", title: "조직, 현장, 전파 채널을 설정합니다.", subtitle: "제출 전에는 필수 설정만 노출하고, 결제와 고급 권한은 후속 범위로 둡니다.", primary: "운영 설정", status: "planned", mappedTo: "/settings · 운영 설정 후보", scope: ["조직/현장", "채널 env 점검", "권한 후속"], metrics: [["조직", "현장"], ["채널", "메일/SMS"]]}
];
