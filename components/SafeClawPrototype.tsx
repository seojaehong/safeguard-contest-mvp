...
const screens: ProductScreen[] = [
  { group: "진입", id: "landing", label: "제품", num: "01", title: "오늘 작업을 안전 문서팩으로.", subtitle: "공식 근거와 현장 전파까지 한 흐름으로 보여주는 대표 진입 화면입니다.", primary: "제품 소개", status: "live", mappedTo: "/ · SafeClawLanding", scope: ["브랜드 첫 진입", "작업 시작", "제품 시연 진입"], metrics: [["CTA", "작업 시작"], ["톤", "안정"]]},
  ...
  { group: "운영", id: "home", label: "홈", num: "03", title: "오늘 처리할 현장을 먼저 보여줍니다.", subtitle: "다현장 상태, API 연결, 미완료 교육, 전파 실패를 한 화면에서 확인합니다.", primary: "현장 현황", status: "planned", mappedTo: "/home · 운영 대시보드", scope: ["다현장 카드", "미완료 교육", "전파 실패 요약"], metrics: [["현장", "다현장"], ["알림", "운영"]]},
  { group: "운영", id: "workspace", label: "작업 입력", num: "04", title: "오늘 작업을 입력하고 문서팩을 생성합니다.", subtitle: "현재 구현된 문서팩 생성 흐름을 SafeClaw 셸로 감싸는 기준 화면입니다.", primary: "문서 생성", status: "live", mappedTo: "/workspace#command · SafeGuardCommandCenter", scope: ["기상 선조회", "API 조합", "문서팩 생성"], metrics: [["문서", "11종"], ["API", "조합"]]},
  { group: "운영", id: "docs", label: "문서팩", num: "05", title: "문서별 편집과 보완 제안을 나란히 둡니다.", subtitle: "위험성평가, 작업계획서, TBM, 안전교육 기록을 제출 서식 기준으로 편집합니다.", primary: "문서 편집", status: "live", mappedTo: "/documents · WorkpackEditor", scope: ["문서별 탭", "보완 문구 생성", "XLS/HWPX/PDF 출력"], metrics: [["정식", "XLS/HWPX/PDF"], ["보완", "삽입"]]},
  ...
  { group: "실행", id: "workers", label: "작업자·교육", num: "07", title: "작업자, 언어, 교육 상태를 한곳에서 봅니다.", subtitle: "연락처, 국적, 주 사용 언어, 신규 여부, 교육 확인 상태를 최소 정보로 관리합니다.", primary: "교육 확인", status: "partial", mappedTo: "/workers · Worker/Education shell", scope: ["작업자 대상", "교육 상태", "언어별 안내"], metrics: [["작업자", "명단"], ["언어", "자동"]]},
  { group: "실행", id: "dispatch", label: "현장 전파", num: "08", title: "메일과 문자 전송 결과를 기록합니다.", subtitle: "카카오와 밴드는 준비 중으로 잠그고, 제출 기준은 메일과 문자로 고정합니다.", primary: "실발송 로그", status: "live", mappedTo: "/dispatch · WorkflowSharePanel", scope: ["n8n dispatch", "메일/SMS", "전파 로그"], metrics: [["메일", "연결"], ["문자", "연결"]]},
  { group: "실행", id: "mobile", label: "작업자 모바일", num: "09", title: "작업자는 핵심 공지만 확인합니다.", subtitle: "작업자용 화면은 길게 설명하지 않고 위험, 조치, 확인만 보여줍니다.", primary: "모바일 확인", status: "planned", mappedTo: "/worker · 작업자 모바일", scope: ["쉬운 한국어", "외국어 전송본", "확인 버튼"], metrics: [["문장", "짧게"], ["언어", "선택"]]},
  { group: "실행", id: "tbm", label: "TBM 모드", num: "10", title: "현장 조회용 TBM 화면입니다.", subtitle: "작업 전 회의에서 크게 띄워 읽는 위험요인, 질문, 확인 항목 중심 화면입니다.", primary: "회의 모드", status: "planned", mappedTo: "/tbm · 풀스크린 TBM", scope: ["TBM 브리핑", "확인 질문", "출석/서명"], metrics: [["질문", "3개"], ["확인", "서명"]]},
  ...
  { group: "시스템", id: "knowledge", label: "지식 DB", num: "12", title: "법령과 공식자료를 지식층으로 관리합니다.", subtitle: "기초 DB와 API 스냅샷을 분리해 최신성과 재현성을 함께 확보합니다.", primary: "지식층", status: "live", mappedTo: "/knowledge · Knowledge DB", scope: ["법령 전문", "KOSHA 자료", "AI 보강"], metrics: [["법령", "전문"], ["자료", "증분"]]},
  { group: "시스템", id: "api", label: "API 상태", num: "13", title: "공공 API 연결 상태를 운영자가 확인합니다.", subtitle: "기상청, Law.go, KOSHA, Work24, Gemini, n8n 연결 상태를 점검합니다.", primary: "연결 점검", status: "live", mappedTo: "/ops/api · API smoke reports", scope: ["dryrun 리포트", "API smoke", "제출 준비 점검"], metrics: [["상태", "채널별"], ["로그", "저장"]]},
    ...
    <main className="safeclaw-prototype-shell" aria-label="SafeClaw 제품 맵">
      ...
      <section className="safeclaw-prototype-main">
        <header className="safeclaw-prototype-topbar">
          ...
          <span>경로 {activeScreen.num}</span>
          <Link href="/workspace">실제 작업공간</Link>
        </header>

        <section className="safeclaw-prototype-stage">
          <aside className="safeclaw-prototype-notice">
            <strong>제품 기능 연결 지도.</strong>
            <p>
              이 페이지는 SafeClaw 기능을 제품 화면 단위로 다시 배치한 내부 지도입니다. 연결된 화면은 바로 열고,
              준비 중인 화면은 현재 가능한 대체 경로를 함께 표시합니다.
            </p>
          </aside>
