...
const screens: ProductScreen[] = [
  ...
  { group: "실행", id: "dispatch", label: "전파 준비", num: "08", title: "채널별 공지 문안과 발송 가능 상태를 확인합니다.", subtitle: "승인된 채널의 접수 결과만 기록하고, 준비되지 않은 채널은 미리보기로 유지합니다.", primary: "전파 상태", status: "partial", mappedTo: "/dispatch · 현장 전파", scope: ["채널 readiness", "메일/SMS 문안", "승인된 결과 기록"], metrics: [["채널", "상태"], ["결과", "승인 후"]]},
  ...
  { group: "시스템", id: "archive", label: "작업 이력", num: "11", title: "과거 문서팩과 발송 이력을 검색합니다.", subtitle: "현장, 날짜, 공정, 작업자, 위험유형 기준으로 이전 기록을 찾습니다.", primary: "이력 검색", status: "partial", mappedTo: "/archive · 작업 이력", scope: ["문서팩 저장", "발송 이력", "기록 재열기"], metrics: [["문서팩", "저장"], ["로그", "발송"]]},
  { group: "시스템", id: "knowledge", label: "지식베이스", num: "12", title: "법령과 공식자료를 지식층으로 관리합니다.", subtitle: "기초 DB와 API 스냅샷을 분리해 최신성과 재현성을 함께 확보합니다.", primary: "지식층", status: "live", mappedTo: "/knowledge · 지식베이스", scope: ["법령 전문", "KOSHA 자료", "AI 보강"], metrics: [["법령", "전문"], ["자료", "증분"]]},
  { group: "시스템", id: "api", label: "연결 상태", num: "13", title: "공공 API 연결 상태를 운영자가 확인합니다.", subtitle: "기상청, Law.go, KOSHA, Work24, Gemini, n8n 연결 상태를 점검합니다.", primary: "연결 점검", status: "live", mappedTo: "/ops/api · 연결 상태", scope: ["운영 점검 로그", "API 연결", "제출 전 점검"], metrics: [["상태", "채널별"], ["로그", "저장"]]},
  { group: "시스템", id: "settings", label: "설정", num: "14", title: "조직, 현장, 발송 채널을 설정합니다.", subtitle: "운영에 필요한 기본 설정과 연결 상태를 한곳에서 확인합니다.", primary: "운영 설정", status: "planned", mappedTo: "/settings · 설정", scope: ["조직/현장", "채널 env 점검", "권한 관리"], metrics: [["조직", "현장"], ["채널", "메일/SMS"]]}
];
    ...
    <main className="safeclaw-prototype-shell" aria-label="SafeClaw 제품 맵">
      ...
      <section className="safeclaw-prototype-main">
        <header className="safeclaw-prototype-topbar">
          ...
          <span>MENU {activeScreen.num}</span>
        ...
        <section className="safeclaw-prototype-stage">
          <aside className="safeclaw-prototype-notice">
            ...
            <p>
              ...
              설정이 필요한 화면은 현재 가능한 업무 경로를 함께 표시합니다.
            </p>
          </aside>
          ...
          <div className="safeclaw-prototype-grid">
            <article className="safeclaw-prototype-card primary">
              ...
              <p>SafeClaw 기능과 연결된 제품 메뉴입니다. 사용 가능한 기능은 업무 화면으로 열고, 설정이 필요한 항목은 연결 상태를 표시합니다.</p>
