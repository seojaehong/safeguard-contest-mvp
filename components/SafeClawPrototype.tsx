...
const screens: ProductScreen[] = [
  ...
  { group: "진입", id: "login", label: "계정", num: "02", title: "관리자 계정으로 이력을 저장합니다.", subtitle: "비회원 임시 저장과 관리자 이력 저장을 명확히 구분합니다.", primary: "이메일 OTP", status: "partial", mappedTo: "/workspace · 저장 상태", scope: ["관리자 저장 안내", "작업 이력 저장", "계정 화면 후속"], metrics: [["저장", "Supabase"], ["모드", "관리자"]]},
  ...
  { group: "실행", id: "evidence", label: "근거", num: "06", title: "법령, KOSHA, 재해사례를 문서 문장에 연결합니다.", subtitle: "인용할 근거와 현재 문서 반영 위치를 함께 보여주는 화면입니다.", primary: "근거 매핑", status: "partial", mappedTo: "/evidence · 근거 라이브러리", scope: ["법령/해석례/판례", "KOSHA 자료", "문서 반영 근거"], metrics: [["법령", "조문"], ["자료", "KOSHA"]]},
  { group: "실행", id: "workers", label: "작업자", num: "07", title: "작업자, 언어, 교육 상태를 한곳에서 봅니다.", subtitle: "연락처, 국적, 주 사용 언어, 신규 여부, 교육 확인 상태를 최소 정보로 관리합니다.", primary: "교육 확인", status: "partial", mappedTo: "/workers · 작업자 관리", scope: ["작업자 대상", "교육 상태", "언어별 안내"], metrics: [["작업자", "명단"], ["언어", "안내"]]},
  { group: "실행", id: "dispatch", label: "전파", num: "08", title: "메일과 문자 전송 결과를 기록합니다.", subtitle: "카카오와 밴드는 승인 상태를 분리하고, 제출 기준은 메일과 문자로 고정합니다.", primary: "실발송 로그", status: "live", mappedTo: "/dispatch · 현장 전파", scope: ["n8n dispatch", "메일/SMS", "전파 로그"], metrics: [["메일", "연결"], ["문자", "연결"]]},
  { group: "실행", id: "mobile", label: "작업자 안내", num: "09", title: "작업자는 핵심 공지만 확인합니다.", subtitle: "작업자용 화면은 길게 설명하지 않고 위험, 조치, 확인만 보여줍니다.", primary: "모바일 확인", status: "planned", mappedTo: "/worker · 작업자 안내", scope: ["쉬운 한국어", "외국어 전송본", "확인 버튼"], metrics: [["문장", "짧게"], ["언어", "선택"]]},
  { group: "실행", id: "tbm", label: "TBM 회의", num: "10", title: "현장 조회용 TBM 화면입니다.", subtitle: "작업 전 회의에서 크게 띄워 읽는 위험요인, 질문, 확인 항목 중심 화면입니다.", primary: "회의 모드", status: "planned", mappedTo: "/tbm · TBM 회의", scope: ["TBM 브리핑", "확인 질문", "출석/서명"], metrics: [["질문", "회의용"], ["확인", "서명"]]},
  { group: "시스템", id: "archive", label: "이력", num: "11", title: "과거 문서팩과 전파 이력을 검색합니다.", subtitle: "현장, 날짜, 공정, 작업자, 위험유형 기준으로 이전 기록을 찾습니다.", primary: "검색 이력", status: "partial", mappedTo: "/archive · 작업 이력", scope: ["문서팩 저장", "전파 이력", "검색 UI 후속"], metrics: [["문서팩", "저장"], ["로그", "전파"]]},
  { group: "시스템", id: "knowledge", label: "지식 DB", num: "12", title: "법령과 공식자료를 지식층으로 관리합니다.", subtitle: "기초 DB와 API 스냅샷을 분리해 최신성과 재현성을 함께 확보합니다.", primary: "지식층", status: "live", mappedTo: "/knowledge · 지식 DB", scope: ["법령 전문", "KOSHA 자료", "AI 보강"], metrics: [["법령", "전문"], ["자료", "증분"]]},
  { group: "시스템", id: "api", label: "API 상태", num: "13", title: "공공 API 연결 상태를 운영자가 확인합니다.", subtitle: "기상청, Law.go, KOSHA, Work24, Gemini, n8n 연결 상태를 점검합니다.", primary: "연결 점검", status: "live", mappedTo: "/ops/api · API 상태", scope: ["운영 점검 로그", "API 연결", "제출 전 점검"], metrics: [["상태", "채널별"], ["로그", "저장"]]},
  { group: "시스템", id: "settings", label: "설정", num: "14", title: "조직, 현장, 전파 채널을 설정합니다.", subtitle: "제출 전에는 필수 설정만 노출하고, 결제와 고급 권한은 후속 범위로 둡니다.", primary: "운영 설정", status: "planned", mappedTo: "/settings · 설정", scope: ["조직/현장", "채널 env 점검", "권한 후속"], metrics: [["조직", "현장"], ["채널", "메일/SMS"]]}
];
      ...
      <section className="safeclaw-prototype-main">
        ...
        <section className="safeclaw-prototype-stage">
          <aside className="safeclaw-prototype-notice">
            <strong>제품 메뉴 연결.</strong>
            <p>
              SafeClaw 기능을 운영 메뉴 단위로 배치했습니다. 바로 사용할 수 있는 화면은 열고,
              제품화 중인 화면은 현재 가능한 업무 경로를 함께 표시합니다.
            </p>
          </aside>
          ...
          <div className="safeclaw-prototype-grid">
            <article className="safeclaw-prototype-card primary">
              ...
              <p>SafeClaw 기능과 연결된 제품 화면입니다. 사용 가능한 기능은 업무 화면으로 열고, 남은 기능은 구현 범위를 표시합니다.</p>
