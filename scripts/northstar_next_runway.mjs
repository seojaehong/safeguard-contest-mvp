...
function launchReadinessSummary(launch) {
  ...
  return {
    ...
    rawAuditFreshnessReasons: Array.isArray(rawAuditFreshness.reasons)
      ? rawAuditFreshness.reasons.map(asString).filter(Boolean)
      : ["raw_audit_freshness_missing"],
...
function ciSupplyChainFullSuiteSummary(report) {
  ...
  return {
    ...
    actionRuntime: asString(upgradedCheckout.runtime),
    packageManagerCache: asBoolean(runtimeUpgrade.packageManagerCache),
    node20DeprecationWarningCount: typeof runtimeUpgrade.node20DeprecationWarningCount === "number"
      ? runtimeUpgrade.node20DeprecationWarningCount
      : null,
    ...
    testsTotal: typeof suite.testsTotal === "number"
      ? suite.testsTotal
      : typeof localSuite.testsTotal === "number"
        ? localSuite.testsTotal
        : null,
...
function freshCurrentSourceSecurityScanSummary(report) {
  ...
  const postScanRemediation = isRecord(report.postScanRemediation) ? report.postScanRemediation : {};
  const remaining = isRecord(report.remainingBoundaries) ? report.remainingBoundaries : {};
  return {
    verdict: asString(report.verdict),
    scanId: asString(report.scanId),
    ...
    approvalFreeProductSourceResidualCount: typeof disposition.approvalFreeProductSourceResidualCount === "number"
      ...
      : null,
    currentApprovalFreeFindingCount: typeof postScanRemediation.remainingApprovalFreeFindingCount === "number"
      ? postScanRemediation.remainingApprovalFreeFindingCount
      : null,
    providerOutputBudgetVerdict: asString(postScanRemediation.providerOutputBudgetVerdict),
    providerOutputBudgetProductCommit: asString(postScanRemediation.providerOutputBudgetProductCommit),
    providerOutputBudgetProductionCommit: asString(postScanRemediation.providerOutputBudgetProductionCommit),
...
/** @param {unknown} report */
function completedCurrentHeadStandardSecurityScanSummary(report) {
  if (!isRecord(report)) return {};
  const scan = isRecord(report.scan) ? report.scan : {};
  const severity = isRecord(scan.severity) ? scan.severity : {};
  const baseline = isRecord(report.baseline) ? report.baseline : {};
  ...
  return {
    ...
    status: asString(scan.status),
    coverageCompleteness: asString(scan.coverageCompleteness),
    recordedSurfaceCount: typeof scan.recordedSurfaceCount === "number" ? scan.recordedSurfaceCount : null,
    deferredCoverageItemCount: typeof scan.deferredCoverageItemCount === "number" ? scan.deferredCoverageItemCount : null,
    reportableFindingCount: typeof scan.reportableFindingCount === "number" ? scan.reportableFindingCount : null,
    mediumFindingCount: typeof severity.medium === "number" ? severity.medium : null,
    lowFindingCount: typeof severity.low === "number" ? severity.low : null,
    immutableOriginalFindingCount: typeof baseline.immutableOriginalFindingCount === "number" ? baseline.immutableOriginalFindingCount : null,
    approvalGatedDatabaseOrAtomicityCount: typeof disposition.approvalGatedDatabaseOrAtomicityCount === "number" ? disposition.approvalGatedDatabaseOrAtomicityCount : null,
    approvalSensitiveShareCapabilityCount: typeof disposition.approvalSensitiveShareCapabilityCount === "number" ? disposition.approvalSensitiveShareCapabilityCount : null,
    approvalFreeProductSourceResidualCount: typeof disposition.approvalFreeProductSourceResidualCount === "number" ? disposition.approvalFreeProductSourceResidualCount : null,
    securityCompleteClaimAllowed: asBoolean(remaining.securityCompleteClaimAllowed),
    exactSavedShareVerdict: asString(remaining.exactSavedShareVerdict),
  };
}
...
/** @param {unknown} report */
function currentSourceForwardedIdentityRemediationSummary(report) {
  if (!isRecord(report)) return {};
  ...
  return {
    ...
    sourceHead: asString(report.sourceHead),
    productionCommit: asString(report.productionCommit),
    sourceIncludedInProduction: asBoolean(report.sourceIncludedInProduction),
    scanId: asString(baseline.scanId),
    findingRule: asString(baseline.findingRule),
    testFileCount: typeof focused.files === "number" ? focused.files : null,
    testCount: typeof focused.tests === "number" ? focused.tests : null,
    liveBehavioralProbeExecuted: asBoolean(verification.liveBehavioralProbeExecuted),
    freshFollowUpSecurityScan: asString(remaining.freshFollowUpSecurityScan),
    securityCompleteClaimAllowed: asBoolean(remaining.securityCompleteClaimAllowed),
    exactSavedShareVerdict: asString(remaining.exactSavedShareVerdict),
  };
}
...
/** @param {unknown} report */
function currentSourceTemplateInventoryRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const baseline = isRecord(report.securityBaseline) ? report.securityBaseline : {};
  const verification = isRecord(report.verification) ? report.verification : {};
  ...
  return {
    ...
    sourceIncludedInProduction: asBoolean(report.sourceIncludedInProduction),
    scanId: asString(baseline.scanId),
    findingId: asString(baseline.findingId),
    findingRule: asString(baseline.findingRule),
    scannerTestCount: typeof scannerTests.tests === "number" ? scannerTests.tests : null,
    archiveSafetyTestCount: typeof archiveTests.tests === "number" ? archiveTests.tests : null,
    liveBehavioralProbeExecuted: asBoolean(verification.liveBehavioralProbeExecuted),
    freshFollowUpSecurityScan: asString(remaining.freshFollowUpSecurityScan),
    securityCompleteClaimAllowed: asBoolean(remaining.securityCompleteClaimAllowed),
    exactSavedShareVerdict: asString(remaining.exactSavedShareVerdict),
...
/** @param {unknown} report */
function currentSourceApprovalFreeSecurityRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const baseline = isRecord(report.scannedBaseline) ? report.scannedBaseline : {};
  ...
  return {
    ...
    productionCommit: asString(report.productionCommit),
    scanId: asString(baseline.scanId),
    reportableFindingCount: typeof baseline.reportableFindingCount === "number" ? baseline.reportableFindingCount : null,
    approvalFreeFindingCount: typeof baseline.approvalFreeFindingCount === "number" ? baseline.approvalFreeFindingCount : null,
    approvalGatedFindingCount: typeof baseline.approvalGatedFindingCount === "number" ? baseline.approvalGatedFindingCount : null,
    currentSourceRemediatedCount: typeof remediation.currentSourceRemediatedCount === "number" ? remediation.currentSourceRemediatedCount : null,
    currentSourceOpenApprovalFreeCount: typeof remediation.currentSourceOpenApprovalFreeCount === "number" ? remediation.currentSourceOpenApprovalFreeCount : null,
    scanFindingReclassificationPerformed: asBoolean(remediation.scanFindingReclassificationPerformed),
    freshFullRepositoryRescanRequired: asBoolean(boundaries.freshFullRepositoryRescanRequired),
    securityCompleteClaimAllowed: asBoolean(boundaries.securityCompleteClaimAllowed),
    exactSavedShareVerdict: asString(boundaries.exactSavedShareVerdict),
  };
}
...
function currentSourceSecurityResourceBudgetRemediationSummary(report) {
  ...
  return {
    ...
    productionCommit: asString(report.productionCommit),
    scanId: asString(baseline.scanId),
    manifestStatus: asString(baseline.manifestStatus),
    canonicalFindingCount: typeof baseline.canonicalFindingCount === "number" ? baseline.canonicalFindingCount : null,
    remediatedFindingCount: Array.isArray(report.remediatedFindings) ? report.remediatedFindings.length : 0,
    approvalGatedFindingCount: Array.isArray(report.remainingApprovalGatedFindings) ? report.remainingApprovalGatedFindings.length : 0,
    directLiveBudgetExecutionProven: asBoolean(live.directLiveBudgetExecutionProven),
...
/** @param {unknown} report */
function currentSourceLogoutStorageRemediationSummary(report) {
  if (!isRecord(report)) return {};
  ...
  return {
    ...
    exactKeyCount: Array.isArray(remediation.clearedExactKeys) ? remediation.clearedExactKeys.length : 0,
    clearedPrefixCount: Array.isArray(remediation.clearedPrefixes) ? remediation.clearedPrefixes.length : 0,
    preservedPreferenceCount: Array.isArray(remediation.preservedPreferenceKeys) ? remediation.preservedPreferenceKeys.length : 0,
    testsPassed: typeof focused.testsPassed === "number" ? focused.testsPassed : null,
    buildStatus: asString(build.status),
    staticViolationCount: typeof staticAudit.violationCount === "number" ? staticAudit.violationCount : null,
    behavioralLogoutExecuted: asBoolean(live.behavioralLogoutExecuted),
...
/** @param {unknown} report */
function currentSourceRawErrorProjectionRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const finding = isRecord(report.finding) ? report.finding : {};
  const remediation = isRecord(report.remediation) ? report.remediation : {};
  const verification = isRecord(report.verification) ? report.verification : {};
  ...
  return {
    ...
    productionCommit: asString(report.productionCommit),
    findingId: asString(finding.findingId),
    stablePublicErrorCodeCount: Array.isArray(remediation.stablePublicErrorCodes) ? remediation.stablePublicErrorCodes.length : 0,
    focusedTestsPassed: typeof focused.testsPassed === "number" ? focused.testsPassed : null,
    adjacentTestsPassed: typeof adjacent.testsPassed === "number" ? adjacent.testsPassed : null,
    buildStatus: asString(build.status),
    liveProbeCount: Array.isArray(live.readOnlyProbes) ? live.readOnlyProbes.length : 0,
    liveFailureInduced: asBoolean(live.liveFailureInduced),
    freshRescanRequired: asBoolean(finding.freshRescanRequired),
    securityComplete: asBoolean(boundaries.securityComplete),
    exactSavedShareVerdict: asString(boundaries.exactSavedShareVerdict),
  };
...
/** @param {unknown} report */
function currentSourceCredentialOutputRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const finding = isRecord(report.finding) ? report.finding : {};
  const remediation = isRecord(report.remediation) ? report.remediation : {};
  const verification = isRecord(report.verification) ? report.verification : {};
  ...
  return {
    ...
    productionCommit: asString(report.productionCommit),
    findingId: asString(finding.findingId),
    credentialCliCount: typeof remediation.credentialCliCount === "number" ? remediation.credentialCliCount : null,
    testsPassed: typeof focused.testsPassed === "number" ? focused.testsPassed : null,
    ciCredentialTestsPassed: typeof ci.credentialOutputTestsPassed === "number" ? ci.credentialOutputTestsPassed : null,
    ciMcpCliTestsPassed: typeof ci.mcpTokenCliTestsPassed === "number" ? ci.mcpTokenCliTestsPassed : null,
    buildStatus: asString(build.status),
    cliProbeCount: Array.isArray(verification.failClosedCliProbes) ? verification.failClosedCliProbes.length : 0,
    credentialIssuanceExecuted: asBoolean(live.credentialIssuanceExecuted),
    freshRescanRequired: asBoolean(finding.freshRescanRequired),
    securityComplete: asBoolean(boundaries.securityComplete),
    exactSavedShareVerdict: asString(boundaries.exactSavedShareVerdict),
  };
...
/** @param {unknown} report */
function currentSourcePhotoReadinessAuthFanoutRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const finding = isRecord(report.finding) ? report.finding : {};
  const remediation = isRecord(report.remediation) ? report.remediation : {};
  const verification = isRecord(report.verification) ? report.verification : {};
  ...
  return {
    ...
    testsPassed: typeof focused.testsPassed === "number" ? focused.testsPassed : null,
    buildStatus: asString(build.status),
    publicGetCreatesSupabaseAdminClient: asBoolean(remediation.publicGetCreatesSupabaseAdminClient),
    publicGetCallsSupabaseAuthentication: asBoolean(remediation.publicGetCallsSupabaseAuthentication),
    responseBodiesEqual: asBoolean(probe.responseBodiesEqual),
    providerDiagnosticsExposed: asBoolean(probe.providerDiagnosticsExposed),
    freshRescanRequired: asBoolean(finding.freshRescanRequired),
    securityComplete: asBoolean(boundaries.securityComplete),
    exactSavedShareVerdict: asString(boundaries.exactSavedShareVerdict),
  };
...
/** @param {unknown} report */
function currentSourceMcpGenerationCancellationRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const finding = isRecord(report.finding) ? report.finding : {};
  const remediation = isRecord(report.remediation) ? report.remediation : {};
  const verification = isRecord(report.verification) ? report.verification : {};
  ...
  return {
    ...
    adjacentTestsPassed: typeof adjacent.testsPassed === "number" ? adjacent.testsPassed : null,
    buildStatus: asString(build.status),
    plainSignalForwarded: asBoolean(remediation.plainGenerationTransportSignalForwarded),
    reviewedSignalForwarded: asBoolean(remediation.reviewedGenerationTransportSignalForwarded),
    persistenceSkippedAfterAbort: asBoolean(remediation.persistenceSkippedAfterAbort),
    authenticatedRuntimeProbeExecuted: asBoolean(live.authenticatedMcpCancellationProbeExecuted),
    freshRescanRequired: asBoolean(finding.freshRescanRequired),
    securityComplete: asBoolean(boundaries.securityComplete),
    exactSavedShareVerdict: asString(boundaries.exactSavedShareVerdict),
  };
...
/** @param {unknown} report */
function staleApprovalEvidenceBindingRemediationSummary(report) {
  if (!isRecord(report)) return {};
  const rows = Array.isArray(report.rows) ? report.rows.filter(isRecord) : [];
  const mutation = isRecord(report.mutationBoundary) ? report.mutationBoundary : {};
  ...
  return {
    verdict: asString(report.verdict),
    sourceHead: asString(report.sourceHead),
    findingId: asString(isRecord(report.finding) ? report.finding.findingId : ""),
    workflowCount: typeof report.workflowCount === "number" ? report.workflowCount : null,
    passedCount: typeof report.passedCount === "number" ? report.passedCount : null,
    failedCount: typeof report.failedCount === "number" ? report.failedCount : null,
    bindingVerifiedCount: rows.filter((row) => row.bindingVerified === true).length,
    blockedWorkflowCount: rows.filter((row) => row.blocked === true).length,
    packetDigests: rows.map((row) => asString(row.packetDigest)).filter(Boolean),
    noMutation: mutation.dbMutationPerformed === false
    ...
    freshFullRepositorySecurityRescanRequired: remaining.freshFullRepositorySecurityRescanRequiredForClosure === true,
    approvalGatedMutationsRemainClosed: remaining.approvalGatedMutationsRemainClosed === true,
    exactSavedShareVerdict: asString(mutation.exactSavedShareVerdict),
  };
}
...
export function buildNorthstarNextRunway(options) {
  ...
  const completedCurrentHeadStandardSecurityScan = readOptionalJson(
    options.rootDir,
    ARTIFACTS.completedCurrentHeadStandardSecurityScan,
  );
  const currentSourceForwardedIdentityRemediation = readOptionalJson(
    options.rootDir,
    ARTIFACTS.currentSourceForwardedIdentityRemediation,
  );
  const currentSourceTemplateInventoryRemediation = readOptionalJson(
    options.rootDir,
    ARTIFACTS.currentSourceTemplateInventoryRemediation,
  );
  const currentSourceApprovalFreeSecurityRemediation = readOptionalJson(
    options.rootDir,
  ...
  const currentSourceLogoutStorageRemediation = readOptionalJson(
    options.rootDir,
  ...
  const currentSourceRawErrorProjectionRemediation = readOptionalJson(
    options.rootDir,
    ARTIFACTS.currentSourceRawErrorProjectionRemediation,
  );
  const currentSourceCredentialOutputRemediation = readOptionalJson(
    options.rootDir,
    ARTIFACTS.currentSourceCredentialOutputRemediation,
  );
  const currentSourcePhotoReadinessAuthFanoutRemediation = readOptionalJson(
    options.rootDir,
    ARTIFACTS.currentSourcePhotoReadinessAuthFanoutRemediation,
  );
  const currentSourceMcpGenerationCancellationRemediation = readOptionalJson(
    options.rootDir,
  ...
  const staleApprovalEvidenceBindingRemediation = readOptionalJson(
    options.rootDir,
    ARTIFACTS.staleApprovalEvidenceBindingRemediation,
  );
  ...
  const completedCurrentHeadStandardSecurityScanResult = completedCurrentHeadStandardSecurityScanSummary(
    completedCurrentHeadStandardSecurityScan,
  );
  const currentSourceForwardedIdentityRemediationResult = currentSourceForwardedIdentityRemediationSummary(
    currentSourceForwardedIdentityRemediation,
  );
  const currentSourceTemplateInventoryRemediationResult = currentSourceTemplateInventoryRemediationSummary(
  ...
  const currentSourceRawErrorProjectionRemediationResult = currentSourceRawErrorProjectionRemediationSummary(
    currentSourceRawErrorProjectionRemediation,
  );
  const currentSourceCredentialOutputRemediationResult = currentSourceCredentialOutputRemediationSummary(
    currentSourceCredentialOutputRemediation,
  );
  const currentSourcePhotoReadinessAuthFanoutRemediationResult = currentSourcePhotoReadinessAuthFanoutRemediationSummary(
  ...
  const staleApprovalEvidenceBindingRemediationResult = staleApprovalEvidenceBindingRemediationSummary(
  ...
  return {
    ...
    provenCurrentState: [
      ...
      ...(ciSupplyChainFullSuiteResult.verdict === "PASS_LIVE_PRODUCTION_GITHUB_CI_NODE24_ACTIONS_FULL_SUITE"
        ...
        && ciSupplyChainFullSuiteResult.testsPassed === 3114
        && ciSupplyChainFullSuiteResult.testsSkipped === 26
        && ciSupplyChainFullSuiteResult.actionRuntime === "node24"
        && ciSupplyChainFullSuiteResult.packageManagerCache === false
        && ciSupplyChainFullSuiteResult.node20DeprecationWarningCount === 0
      ...
      ...(staleApprovalEvidenceBindingRemediationResult.verdict === "PASS_CURRENT_SOURCE_STALE_APPROVAL_EVIDENCE_BINDING_FAIL_CLOSED"
        && staleApprovalEvidenceBindingRemediationResult.workflowCount === 4
        && staleApprovalEvidenceBindingRemediationResult.passedCount === 4
        && staleApprovalEvidenceBindingRemediationResult.failedCount === 0
        && staleApprovalEvidenceBindingRemediationResult.bindingVerifiedCount === 4
        && staleApprovalEvidenceBindingRemediationResult.blockedWorkflowCount === 4
        && staleApprovalEvidenceBindingRemediationResult.noMutation === true
        && staleApprovalEvidenceBindingRemediationResult.freshFullRepositorySecurityRescanRequired === true
        && staleApprovalEvidenceBindingRemediationResult.approvalGatedMutationsRemainClosed === true
        && staleApprovalEvidenceBindingRemediationResult.exactSavedShareVerdict === "MISSING_EVIDENCE"
        ? ["stale_approval_evidence_binding_security"]
        : []),
    ...
    noticeState: [
      ...
      },
      {
        ...
        state: "notice",
        reason: `fresh Standard scan ${freshCurrentSourceSecurityScanResult.scanId || "missing"} records ${freshCurrentSourceSecurityScanResult.reportableFindingCount ?? "unknown"} open findings with ${freshCurrentSourceSecurityScanResult.coverageCompleteness || "unknown"} coverage; scan-time/current approval-free source residuals are ${freshCurrentSourceSecurityScanResult.approvalFreeProductSourceResidualCount ?? "unknown"}/${freshCurrentSourceSecurityScanResult.currentApprovalFreeFindingCount ?? "unknown"}, provider-output remediation is ${freshCurrentSourceSecurityScanResult.providerOutputBudgetVerdict || "missing"} at production ${freshCurrentSourceSecurityScanResult.providerOutputBudgetProductionCommit?.slice(0, 8) || "missing"}, ${freshCurrentSourceSecurityScanResult.approvalSensitiveShareCapabilityCount ?? "unknown"} separately reportable Share capability findings and ${freshCurrentSourceSecurityScanResult.approvalGatedDatabaseOrAtomicityCount ?? "unknown"} database/RLS/atomicity findings remain open; security-complete is false and exact saved Share remains ${freshCurrentSourceSecurityScanResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
        gate: "completed_current_head_standard_security_scan",
        state: "notice",
        reason: `current-head Standard scan ${completedCurrentHeadStandardSecurityScanResult.scanId || "missing"} records ${completedCurrentHeadStandardSecurityScanResult.reportableFindingCount ?? "unknown"} findings (${completedCurrentHeadStandardSecurityScanResult.mediumFindingCount ?? "unknown"} medium, ${completedCurrentHeadStandardSecurityScanResult.lowFindingCount ?? "unknown"} low) with ${completedCurrentHeadStandardSecurityScanResult.coverageCompleteness || "unknown"} coverage; the immutable ${completedCurrentHeadStandardSecurityScanResult.immutableOriginalFindingCount ?? "unknown"}-finding baseline remains preserved, ${completedCurrentHeadStandardSecurityScanResult.approvalFreeProductSourceResidualCount ?? "unknown"} approval-free findings remain open, security-complete is false, and exact saved Share remains ${completedCurrentHeadStandardSecurityScanResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
        gate: "current_source_forwarded_identity_remediation",
        state: "proven",
        reason: `deployed source ${currentSourceForwardedIdentityRemediationResult.sourceHead?.slice(0, 8) || "missing"} verifies trusted Vercel ingress before using forwarded identity with ${currentSourceForwardedIdentityRemediationResult.testFileCount ?? "unknown"} files / ${currentSourceForwardedIdentityRemediationResult.testCount ?? "unknown"} tests; the sealed scan remains immutable, fresh scan is ${currentSourceForwardedIdentityRemediationResult.freshFollowUpSecurityScan || "REQUIRED"}, security-complete is false, and exact saved Share remains ${currentSourceForwardedIdentityRemediationResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
        gate: "current_source_template_inventory_remediation",
        state: "proven",
        reason: `deployed source ${currentSourceTemplateInventoryRemediationResult.sourceHead?.slice(0, 8) || "missing"} bounds local template inventory before parser initialization with scanner/archive tests ${currentSourceTemplateInventoryRemediationResult.scannerTestCount ?? "unknown"}/${currentSourceTemplateInventoryRemediationResult.archiveSafetyTestCount ?? "unknown"}; the sealed scan remains immutable, fresh scan is ${currentSourceTemplateInventoryRemediationResult.freshFollowUpSecurityScan || "REQUIRED"}, security-complete is false, and exact saved Share remains ${currentSourceTemplateInventoryRemediationResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
      ...
      },
      {
        ...
        state: "notice",
        reason: `live source replaces raw database/provider/webhook/file-I/O errors with ${currentSourceRawErrorProjectionRemediationResult.stablePublicErrorCodeCount ?? "unknown"} stable public codes; focused/adjacent tests ${currentSourceRawErrorProjectionRemediationResult.focusedTestsPassed ?? "unknown"}/${currentSourceRawErrorProjectionRemediationResult.adjacentTestsPassed ?? "unknown"}, build=${currentSourceRawErrorProjectionRemediationResult.buildStatus || "missing"}, live probes=${currentSourceRawErrorProjectionRemediationResult.liveProbeCount ?? "unknown"}. Production failure induced=${currentSourceRawErrorProjectionRemediationResult.liveFailureInduced === true}; sealed finding fresh-rescan pending=${currentSourceRawErrorProjectionRemediationResult.freshRescanRequired === true}, security-complete=${currentSourceRawErrorProjectionRemediationResult.securityComplete === true}, and exact saved Share remains ${currentSourceRawErrorProjectionRemediationResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
        gate: "current_source_credential_output_remediation",
        state: "notice",
        reason: `live source protects ${currentSourceCredentialOutputRemediationResult.credentialCliCount ?? "unknown"} credential CLIs with explicit output modes; tests=${currentSourceCredentialOutputRemediationResult.testsPassed ?? "unknown"}, CI credential/MCP=${currentSourceCredentialOutputRemediationResult.ciCredentialTestsPassed ?? "unknown"}/${currentSourceCredentialOutputRemediationResult.ciMcpCliTestsPassed ?? "unknown"}, build=${currentSourceCredentialOutputRemediationResult.buildStatus || "missing"}, fail-closed probes=${currentSourceCredentialOutputRemediationResult.cliProbeCount ?? "unknown"}. Credential issuance executed=${currentSourceCredentialOutputRemediationResult.credentialIssuanceExecuted === true}; sealed finding fresh-rescan pending=${currentSourceCredentialOutputRemediationResult.freshRescanRequired === true}, security-complete=${currentSourceCredentialOutputRemediationResult.securityComplete === true}, and exact saved Share remains ${currentSourceCredentialOutputRemediationResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
        gate: "current_source_photo_readiness_auth_fanout_remediation",
        state: "notice",
        reason: `live source keeps public photo readiness GET auth-free and coarse: Supabase client=${currentSourcePhotoReadinessAuthFanoutRemediationResult.publicGetCreatesSupabaseAdminClient === true}, auth lookup=${currentSourcePhotoReadinessAuthFanoutRemediationResult.publicGetCallsSupabaseAuthentication === true}, live anonymous/Bearer bodies equal=${currentSourcePhotoReadinessAuthFanoutRemediationResult.responseBodiesEqual === true}, diagnostics exposed=${currentSourcePhotoReadinessAuthFanoutRemediationResult.providerDiagnosticsExposed === true}; ${currentSourcePhotoReadinessAuthFanoutRemediationResult.testsPassed ?? "unknown"} tests and build=${currentSourcePhotoReadinessAuthFanoutRemediationResult.buildStatus || "missing"}. The sealed finding remains fresh-rescan pending=${currentSourcePhotoReadinessAuthFanoutRemediationResult.freshRescanRequired === true}, security-complete=${currentSourcePhotoReadinessAuthFanoutRemediationResult.securityComplete === true}, and exact saved Share remains ${currentSourcePhotoReadinessAuthFanoutRemediationResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
      ...
      },
      {
        ...
        state: "proven",
        reason: `current live source binds ${staleApprovalEvidenceBindingRemediationResult.bindingVerifiedCount ?? "unknown"}/${staleApprovalEvidenceBindingRemediationResult.workflowCount ?? "unknown"} approval workflows to current HEAD, production ancestry, required-input SHA-256 values, and deterministic packet digests; all ${staleApprovalEvidenceBindingRemediationResult.blockedWorkflowCount ?? "unknown"} remain fail-closed, no mutation=${staleApprovalEvidenceBindingRemediationResult.noMutation === true}, fresh full rescan required=${staleApprovalEvidenceBindingRemediationResult.freshFullRepositorySecurityRescanRequired === true}, and exact saved Share remains ${staleApprovalEvidenceBindingRemediationResult.exactSavedShareVerdict || "MISSING_EVIDENCE"}`,
      },
      {
    ...
    completedCurrentHeadStandardSecurityScan: completedCurrentHeadStandardSecurityScanResult,
    ...
    currentSourceRawErrorProjectionRemediation: currentSourceRawErrorProjectionRemediationResult,
    currentSourceCredentialOutputRemediation: currentSourceCredentialOutputRemediationResult,
    ...
    staleApprovalEvidenceBindingRemediation: staleApprovalEvidenceBindingRemediationResult,
...
- Pinned CI supply-chain/full-suite proof is \`${report.ciSupplyChainFullSuite.verdict || "missing"}\`: GitHub run \`${report.ciSupplyChainFullSuite.githubRunId ?? "missing"}\` concluded \`${report.ciSupplyChainFullSuite.githubConclusion || "missing"}\` with \`${report.ciSupplyChainFullSuite.testsPassed ?? "unknown"}\` tests passed, \`${report.ciSupplyChainFullSuite.testsSkipped ?? "unknown"}\` skipped, build \`${report.ciSupplyChainFullSuite.build || "missing"}\`, action runtime \`${report.ciSupplyChainFullSuite.actionRuntime || "missing"}\`, package-manager cache \`${report.ciSupplyChainFullSuite.packageManagerCache === true}\`, and Node 20 warnings \`${report.ciSupplyChainFullSuite.node20DeprecationWarningCount ?? "unknown"}\`. Checkout/setup-node remain immutable SHAs. This does not close unrelated security findings or approval-gated runtime work; exact saved Share remains \`${report.ciSupplyChainFullSuite.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
...
- Fresh current-source Standard security scan is \`${report.freshCurrentSourceSecurityScan.verdict || "missing"}\`: scan \`${report.freshCurrentSourceSecurityScan.scanId || "missing"}\`, findings \`${report.freshCurrentSourceSecurityScan.reportableFindingCount ?? "unknown"}\` (medium/low \`${report.freshCurrentSourceSecurityScan.mediumFindingCount ?? "unknown"}/${report.freshCurrentSourceSecurityScan.lowFindingCount ?? "unknown"}\`), coverage \`${report.freshCurrentSourceSecurityScan.coverageCompleteness || "unknown"}\` with \`${report.freshCurrentSourceSecurityScan.reviewedSurfaceCount ?? "unknown"}\` recorded surfaces and \`${report.freshCurrentSourceSecurityScan.deferredCoverageItemCount ?? "unknown"}\` deferred items. Scan-time/current approval-free residuals are \`${report.freshCurrentSourceSecurityScan.approvalFreeProductSourceResidualCount ?? "unknown"}/${report.freshCurrentSourceSecurityScan.currentApprovalFreeFindingCount ?? "unknown"}\`; provider-output remediation is \`${report.freshCurrentSourceSecurityScan.providerOutputBudgetVerdict || "missing"}\` at production \`${report.freshCurrentSourceSecurityScan.providerOutputBudgetProductionCommit?.slice(0, 8) || "missing"}\`. Database/atomicity and Share capability boundaries remain \`${report.freshCurrentSourceSecurityScan.approvalGatedDatabaseOrAtomicityCount ?? "unknown"}/${report.freshCurrentSourceSecurityScan.approvalSensitiveShareCapabilityCount ?? "unknown"}\`. Scan completion is not security-complete \`${report.freshCurrentSourceSecurityScan.securityCompleteClaimAllowed === true}\`, and exact saved Share remains \`${report.freshCurrentSourceSecurityScan.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
- Stale approval evidence binding remediation is \`${report.staleApprovalEvidenceBindingRemediation.verdict || "missing"}\`: finding \`${report.staleApprovalEvidenceBindingRemediation.findingId || "missing"}\`, bound workflows \`${report.staleApprovalEvidenceBindingRemediation.bindingVerifiedCount ?? "unknown"}/${report.staleApprovalEvidenceBindingRemediation.workflowCount ?? "unknown"}\`, blocked workflows \`${report.staleApprovalEvidenceBindingRemediation.blockedWorkflowCount ?? "unknown"}\`, and packet digests \`${report.staleApprovalEvidenceBindingRemediation.packetDigests?.length ?? "unknown"}\`. No mutation remains \`${report.staleApprovalEvidenceBindingRemediation.noMutation === true}\`, a fresh full-repository rescan remains required \`${report.staleApprovalEvidenceBindingRemediation.freshFullRepositorySecurityRescanRequired === true}\`, approval-gated mutations remain closed \`${report.staleApprovalEvidenceBindingRemediation.approvalGatedMutationsRemainClosed === true}\`, and exact saved Share remains \`${report.staleApprovalEvidenceBindingRemediation.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
- Completed current-head Standard security scan is \`${report.completedCurrentHeadStandardSecurityScan.verdict || "missing"}\`: scan \`${report.completedCurrentHeadStandardSecurityScan.scanId || "missing"}\`, source \`${report.completedCurrentHeadStandardSecurityScan.sourceHead || "missing"}\`, findings \`${report.completedCurrentHeadStandardSecurityScan.reportableFindingCount ?? "unknown"}\` (medium/low \`${report.completedCurrentHeadStandardSecurityScan.mediumFindingCount ?? "unknown"}/${report.completedCurrentHeadStandardSecurityScan.lowFindingCount ?? "unknown"}\`), coverage \`${report.completedCurrentHeadStandardSecurityScan.coverageCompleteness || "unknown"}\` with \`${report.completedCurrentHeadStandardSecurityScan.recordedSurfaceCount ?? "unknown"}\` recorded rows and \`${report.completedCurrentHeadStandardSecurityScan.deferredCoverageItemCount ?? "unknown"}\` deferred rows. The immutable \`${report.completedCurrentHeadStandardSecurityScan.immutableOriginalFindingCount ?? "unknown"}\`-finding baseline remains preserved; approval-free/database/Share residuals remain \`${report.completedCurrentHeadStandardSecurityScan.approvalFreeProductSourceResidualCount ?? "unknown"}/${report.completedCurrentHeadStandardSecurityScan.approvalGatedDatabaseOrAtomicityCount ?? "unknown"}/${report.completedCurrentHeadStandardSecurityScan.approvalSensitiveShareCapabilityCount ?? "unknown"}\`. Security-complete remains \`${report.completedCurrentHeadStandardSecurityScan.securityCompleteClaimAllowed === true}\`, and exact saved Share remains \`${report.completedCurrentHeadStandardSecurityScan.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
...
- Current-source raw error projection remediation is \`${report.currentSourceRawErrorProjectionRemediation.verdict || "missing"}\`: product/live \`${report.currentSourceRawErrorProjectionRemediation.sourceHead || "missing"}/${report.currentSourceRawErrorProjectionRemediation.productionCommit || "missing"}\`, stable public codes \`${report.currentSourceRawErrorProjectionRemediation.stablePublicErrorCodeCount ?? "unknown"}\`, focused/adjacent tests \`${report.currentSourceRawErrorProjectionRemediation.focusedTestsPassed ?? "unknown"}/${report.currentSourceRawErrorProjectionRemediation.adjacentTestsPassed ?? "unknown"}\`, build \`${report.currentSourceRawErrorProjectionRemediation.buildStatus || "missing"}\`, and live probes \`${report.currentSourceRawErrorProjectionRemediation.liveProbeCount ?? "unknown"}\`. No production failure was induced \`${report.currentSourceRawErrorProjectionRemediation.liveFailureInduced === true}\`; fresh rescan required remains \`${report.currentSourceRawErrorProjectionRemediation.freshRescanRequired === true}\`, security-complete remains \`${report.currentSourceRawErrorProjectionRemediation.securityComplete === true}\`, and exact saved Share remains \`${report.currentSourceRawErrorProjectionRemediation.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
- Current-source credential output remediation is \`${report.currentSourceCredentialOutputRemediation.verdict || "missing"}\`: product/live \`${report.currentSourceCredentialOutputRemediation.sourceHead || "missing"}/${report.currentSourceCredentialOutputRemediation.productionCommit || "missing"}\`, credential CLIs \`${report.currentSourceCredentialOutputRemediation.credentialCliCount ?? "unknown"}\`, focused tests \`${report.currentSourceCredentialOutputRemediation.testsPassed ?? "unknown"}\`, CI credential/MCP tests \`${report.currentSourceCredentialOutputRemediation.ciCredentialTestsPassed ?? "unknown"}/${report.currentSourceCredentialOutputRemediation.ciMcpCliTestsPassed ?? "unknown"}\`, build \`${report.currentSourceCredentialOutputRemediation.buildStatus || "missing"}\`, and fail-closed probes \`${report.currentSourceCredentialOutputRemediation.cliProbeCount ?? "unknown"}\`. Credential issuance executed remains \`${report.currentSourceCredentialOutputRemediation.credentialIssuanceExecuted === true}\`; fresh rescan required remains \`${report.currentSourceCredentialOutputRemediation.freshRescanRequired === true}\`, security-complete remains \`${report.currentSourceCredentialOutputRemediation.securityComplete === true}\`, and exact saved Share remains \`${report.currentSourceCredentialOutputRemediation.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
