...
function knowledgeViewportWorkbenchSummary(report) {
  ...
  return {
    ...
    technicalDisclosureCount: typeof referenceDisclosure.technicalDisclosureCount === "number" ? referenceDisclosure.technicalDisclosureCount : 0,
    referenceDisclosureCount: typeof referenceDisclosure.referenceDisclosureCount === "number" ? referenceDisclosure.referenceDisclosureCount : 0,
    defaultOpenDisclosureCount: typeof referenceDisclosure.defaultOpenDisclosureCount === "number" ? referenceDisclosure.defaultOpenDisclosureCount : 0,
    exclusiveDisclosureGroups: asBoolean(referenceDisclosure.exclusiveDisclosureGroups),
    maxMobileTechnicalScrollRatio: typeof referenceDisclosure.maxMobileTechnicalScrollRatio === "number" ? referenceDisclosure.maxMobileTechnicalScrollRatio : 0,
    maxMobileReferenceScrollRatio: typeof referenceDisclosure.maxMobileReferenceScrollRatio === "number" ? referenceDisclosure.maxMobileReferenceScrollRatio : 0,
    firstDisclosureInsidePanel: asBoolean(referenceDisclosure.firstDisclosureInsidePanel),
    exactSavedShareVerdict: asString(remainingBoundaries.exactSavedShareVerdict),
...
- Live Knowledge viewport workbench is measured separately: \`${report.knowledgeViewportWorkbench.verdict || "missing"}\`; browser rows \`${report.knowledgeViewportWorkbench.passCount ?? 0}/${report.knowledgeViewportWorkbench.rowCount ?? 0}\`, maximum body ratio \`${report.knowledgeViewportWorkbench.maxBodyRatio ?? 0}\`, selected exposure \`${report.knowledgeViewportWorkbench.visiblePanelCountPerRow ?? 0}\` visible panel and \`${report.knowledgeViewportWorkbench.reachableSectionCountPerRow ?? 0}\` reachable tasks. KOSHA technical/reference disclosures are \`${report.knowledgeViewportWorkbench.technicalDisclosureCount ?? 0}/${report.knowledgeViewportWorkbench.referenceDisclosureCount ?? 0}\`, default open \`${report.knowledgeViewportWorkbench.defaultOpenDisclosureCount ?? 0}\`, exclusive groups \`${report.knowledgeViewportWorkbench.exclusiveDisclosureGroups === true}\`, mobile ratios \`${report.knowledgeViewportWorkbench.maxMobileTechnicalScrollRatio ?? 0}/${report.knowledgeViewportWorkbench.maxMobileReferenceScrollRatio ?? 0}\`, and first item panel-contained \`${report.knowledgeViewportWorkbench.firstDisclosureInsidePanel === true}\`. Route splitting alone is not treated as the fix; long content remains in local-scroll panels. Exact saved Share remains \`${report.knowledgeViewportWorkbench.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`, Wiki publication remains \`${report.knowledgeViewportWorkbench.llmWikiPublicationVerdict || "APPROVAL_GATED"}\`, and SIF embedding remains \`${report.knowledgeViewportWorkbench.sifEmbeddingRuntimeVerdict || "APPROVAL_GATED"}\`.
