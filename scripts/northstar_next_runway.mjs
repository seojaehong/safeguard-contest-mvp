    ...
    noticeState: [
      ...
      },
      {
        ...
        reason: mcpGenerationWorkBudgetSecurityResult.distributedActivationRequired
          ? "deployed source includes the measured MCP body and token-bound admission budgets; a valid authenticated runtime probe, distributed activation, and fresh rescan remain open"
          : "distributed MCP admission is activated but currently unhealthy and fails closed before authentication; backend health, a valid authenticated runtime probe, and fresh rescan remain open",
      },
      {
...
- MCP generation work-budget security is separately measured: \`${report.mcpGenerationWorkBudgetSecurity.verdict || "missing"}\`, POST body budget \`${report.mcpGenerationWorkBudgetSecurity.postBodyMaxBytes ?? "unknown"}\` bytes, adjacent tests \`${report.mcpGenerationWorkBudgetSecurity.adjacentTests ?? "unknown"}\`, valid authenticated runtime probe pending=\`${report.mcpGenerationWorkBudgetSecurity.validAuthenticatedRuntimeProbeRequired === true}\`, distributed activation pending=\`${report.mcpGenerationWorkBudgetSecurity.distributedActivationRequired === true}\`, distributed backend health pending=\`${report.mcpGenerationWorkBudgetSecurity.distributedHealthRequired === true}\`, current refresh status/mode/error=\`${report.mcpGenerationWorkBudgetSecurity.currentRefreshStatus ?? "unknown"}/${report.mcpGenerationWorkBudgetSecurity.currentRefreshRateLimitMode || "unknown"}/${report.mcpGenerationWorkBudgetSecurity.currentRefreshErrorCode || "none"}\`, and fresh rescan required=\`${report.mcpGenerationWorkBudgetSecurity.freshRescanRequired === true}\`. This notice preserves the sealed finding and exact saved Share \`${report.mcpGenerationWorkBudgetSecurity.exactSavedShareVerdict || "MISSING_EVIDENCE"}\`.
