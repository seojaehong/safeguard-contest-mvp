...
export function buildNorthstarLiveRollup(rootDir, buildInfo, generatedAt = new Date().toISOString()) {
  ...
  return {
    ...
    knowledgeViewportWorkbench: {
      ...
      technicalDisclosureCount: asNumber(recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.technicalDisclosureCount),
      referenceDisclosureCount: asNumber(recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.referenceDisclosureCount),
      defaultOpenDisclosureCount: asNumber(recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.defaultOpenDisclosureCount),
      exclusiveDisclosureGroups: recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.exclusiveDisclosureGroups === true,
      maxMobileTechnicalScrollRatio: asNumber(recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.maxMobileTechnicalScrollRatio),
      maxMobileReferenceScrollRatio: asNumber(recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.maxMobileReferenceScrollRatio),
      firstDisclosureInsidePanel: recordAt(recordAt(knowledgeViewportWorkbench, "browser"), "referenceDisclosure")?.firstDisclosureInsidePanel === true,
...
export function renderNorthstarLiveRollupMarkdown(rollup) {
  const lines = [
    ...
    `- KOSHA references: ${rollup.knowledgeViewportWorkbench.technicalDisclosureCount ?? 0}/${rollup.knowledgeViewportWorkbench.referenceDisclosureCount ?? 0} technical/reference disclosures, default open=${rollup.knowledgeViewportWorkbench.defaultOpenDisclosureCount ?? 0}, exclusive groups=${rollup.knowledgeViewportWorkbench.exclusiveDisclosureGroups === true}, mobile ratios=${rollup.knowledgeViewportWorkbench.maxMobileTechnicalScrollRatio ?? 0}/${rollup.knowledgeViewportWorkbench.maxMobileReferenceScrollRatio ?? 0}, first item panel-contained=${rollup.knowledgeViewportWorkbench.firstDisclosureInsidePanel === true}.`,
