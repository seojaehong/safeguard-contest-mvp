...
export function buildNorthstarLiveRollup(rootDir, buildInfo, generatedAt = new Date().toISOString()) {
  ...
  const hermesCandidateBodyHazardCoverage = tryReadJson(rootDir, ARTIFACTS.hermesCandidateBodyHazardCoverage);
  ...
  const currentSourceRawErrorProjectionRemediation = tryReadJson(rootDir, ARTIFACTS.currentSourceRawErrorProjectionRemediation);
  const currentSourceCredentialOutputRemediation = tryReadJson(rootDir, ARTIFACTS.currentSourceCredentialOutputRemediation);
  ...
  const evidence = [
    ...
    evidenceStatus(
      ...
      currentHead,
      liveCommit,
      "hermes_candidate_body_hazard_coverage",
      ARTIFACTS.hermesCandidateBodyHazardCoverage,
      isRecord(hermesCandidateBodyHazardCoverage)
        ? {
            ...hermesCandidateBodyHazardCoverage,
            sourceHead: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.sourceHead),
            productionCommit: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.productionCommit),
          }
        : hermesCandidateBodyHazardCoverage,
    ),
    ...
    evidenceStatus(rootDir, currentHead, liveCommit, "current_source_raw_error_projection_remediation", ARTIFACTS.currentSourceRawErrorProjectionRemediation, currentSourceRawErrorProjectionRemediation),
    evidenceStatus(rootDir, currentHead, liveCommit, "current_source_credential_output_remediation", ARTIFACTS.currentSourceCredentialOutputRemediation, currentSourceCredentialOutputRemediation),
  ...
  return {
    ...
    freshCurrentSourceSecurityScan: {
      ...
      currentApprovalFreeFindingCount: asNumber(recordAt(freshCurrentSourceSecurityScan, "postScanRemediation")?.remainingApprovalFreeFindingCount),
      providerOutputBudgetVerdict: asString(recordAt(freshCurrentSourceSecurityScan, "postScanRemediation")?.providerOutputBudgetVerdict),
      providerOutputBudgetProductCommit: asString(recordAt(freshCurrentSourceSecurityScan, "postScanRemediation")?.providerOutputBudgetProductCommit),
      providerOutputBudgetProductionCommit: asString(recordAt(freshCurrentSourceSecurityScan, "postScanRemediation")?.providerOutputBudgetProductionCommit),
    ...
    completedCurrentHeadStandardSecurityScan: {
      artifact: ARTIFACTS.completedCurrentHeadStandardSecurityScan,
      verdict: isRecord(completedCurrentHeadStandardSecurityScan) ? asString(completedCurrentHeadStandardSecurityScan.verdict) : "missing",
      scanId: isRecord(completedCurrentHeadStandardSecurityScan) ? asString(completedCurrentHeadStandardSecurityScan.scanId) : "",
      sourceHead: isRecord(completedCurrentHeadStandardSecurityScan) ? asString(completedCurrentHeadStandardSecurityScan.sourceHead) : "",
      reportableFindingCount: asNumber(recordAt(completedCurrentHeadStandardSecurityScan, "scan")?.reportableFindingCount),
      mediumFindingCount: asNumber(recordAt(recordAt(completedCurrentHeadStandardSecurityScan, "scan"), "severity")?.medium),
      lowFindingCount: asNumber(recordAt(recordAt(completedCurrentHeadStandardSecurityScan, "scan"), "severity")?.low),
      coverageCompleteness: asString(recordAt(completedCurrentHeadStandardSecurityScan, "scan")?.coverageCompleteness),
      recordedSurfaceCount: asNumber(recordAt(completedCurrentHeadStandardSecurityScan, "scan")?.recordedSurfaceCount),
      deferredCoverageItemCount: asNumber(recordAt(completedCurrentHeadStandardSecurityScan, "scan")?.deferredCoverageItemCount),
      immutableOriginalFindingCount: asNumber(recordAt(completedCurrentHeadStandardSecurityScan, "baseline")?.immutableOriginalFindingCount),
      approvalGatedDatabaseOrAtomicityCount: asNumber(recordAt(completedCurrentHeadStandardSecurityScan, "currentDisposition")?.approvalGatedDatabaseOrAtomicityCount),
      approvalSensitiveShareCapabilityCount: asNumber(recordAt(completedCurrentHeadStandardSecurityScan, "currentDisposition")?.approvalSensitiveShareCapabilityCount),
    ...
    currentSourceOntologyErrorProjectionRemediation: {
      artifact: ARTIFACTS.currentSourceOntologyErrorProjectionRemediation,
      verdict: isRecord(currentSourceOntologyErrorProjectionRemediation)
        ? asString(currentSourceOntologyErrorProjectionRemediation.verdict)
        : "missing",
      sourceHead: isRecord(currentSourceOntologyErrorProjectionRemediation)
        ? asString(currentSourceOntologyErrorProjectionRemediation.sourceHead)
        : "",
      productionCommit: isRecord(currentSourceOntologyErrorProjectionRemediation)
        ? asString(currentSourceOntologyErrorProjectionRemediation.productionCommit)
        : "",
      findingId: asString(recordAt(currentSourceOntologyErrorProjectionRemediation, "finding")?.findingId),
      testsPassed: asNumber(recordAt(recordAt(currentSourceOntologyErrorProjectionRemediation, "verification"), "focusedAndAdjacentTests")?.testsPassed),
      buildStatus: asString(recordAt(recordAt(currentSourceOntologyErrorProjectionRemediation, "verification"), "productionBuild")?.status),
      publicErrorCodeCount: Array.isArray(recordAt(currentSourceOntologyErrorProjectionRemediation, "remediation")?.publicErrorCodes)
        ? recordAt(currentSourceOntologyErrorProjectionRemediation, "remediation").publicErrorCodes.length
        : 0,
      upstreamFailureInduced: recordAt(recordAt(currentSourceOntologyErrorProjectionRemediation, "verification"), "liveDeployment")?.upstreamFailureInduced === true,
      liveProbeCode: asString(recordAt(recordAt(recordAt(currentSourceOntologyErrorProjectionRemediation, "verification"), "liveDeployment"), "publicProbe")?.code),
      freshRescanRequired: recordAt(currentSourceOntologyErrorProjectionRemediation, "finding")?.freshRescanRequired === true,
      securityComplete: recordAt(currentSourceOntologyErrorProjectionRemediation, "remainingBoundaries")?.securityComplete === true,
      exactSavedShareVerdict: asString(recordAt(currentSourceOntologyErrorProjectionRemediation, "remainingBoundaries")?.exactSavedShareVerdict),
    },
    currentSourceRawErrorProjectionRemediation: {
      artifact: ARTIFACTS.currentSourceRawErrorProjectionRemediation,
      verdict: isRecord(currentSourceRawErrorProjectionRemediation)
        ? asString(currentSourceRawErrorProjectionRemediation.verdict)
        : "missing",
      sourceHead: isRecord(currentSourceRawErrorProjectionRemediation)
        ? asString(currentSourceRawErrorProjectionRemediation.sourceHead)
        : "",
      productionCommit: isRecord(currentSourceRawErrorProjectionRemediation)
        ? asString(currentSourceRawErrorProjectionRemediation.productionCommit)
        : "",
      findingId: asString(recordAt(currentSourceRawErrorProjectionRemediation, "finding")?.findingId),
      stablePublicErrorCodeCount: Array.isArray(recordAt(currentSourceRawErrorProjectionRemediation, "remediation")?.stablePublicErrorCodes)
        ? recordAt(currentSourceRawErrorProjectionRemediation, "remediation").stablePublicErrorCodes.length
        : 0,
      focusedTestsPassed: asNumber(recordAt(recordAt(currentSourceRawErrorProjectionRemediation, "verification"), "focusedSecurityTests")?.testsPassed),
      adjacentTestsPassed: asNumber(recordAt(recordAt(currentSourceRawErrorProjectionRemediation, "verification"), "adjacentRegressionTests")?.testsPassed),
      liveProbeCount: Array.isArray(recordAt(recordAt(currentSourceRawErrorProjectionRemediation, "verification"), "liveDeployment")?.readOnlyProbes)
        ? recordAt(recordAt(currentSourceRawErrorProjectionRemediation, "verification"), "liveDeployment").readOnlyProbes.length
        : 0,
      liveFailureInduced: recordAt(recordAt(currentSourceRawErrorProjectionRemediation, "verification"), "liveDeployment")?.liveFailureInduced === true,
      freshRescanRequired: recordAt(currentSourceRawErrorProjectionRemediation, "finding")?.freshRescanRequired === true,
      securityComplete: recordAt(currentSourceRawErrorProjectionRemediation, "remainingBoundaries")?.securityComplete === true,
      exactSavedShareVerdict: asString(recordAt(currentSourceRawErrorProjectionRemediation, "remainingBoundaries")?.exactSavedShareVerdict),
    },
    currentSourceCredentialOutputRemediation: {
      artifact: ARTIFACTS.currentSourceCredentialOutputRemediation,
      verdict: isRecord(currentSourceCredentialOutputRemediation) ? asString(currentSourceCredentialOutputRemediation.verdict) : null,
      sourceHead: isRecord(currentSourceCredentialOutputRemediation) ? asString(currentSourceCredentialOutputRemediation.sourceHead) : null,
      productionCommit: isRecord(currentSourceCredentialOutputRemediation) ? asString(currentSourceCredentialOutputRemediation.productionCommit) : null,
      findingId: asString(recordAt(currentSourceCredentialOutputRemediation, "finding")?.findingId),
      credentialCliCount: asNumber(recordAt(currentSourceCredentialOutputRemediation, "remediation")?.credentialCliCount),
      testsPassed: asNumber(recordAt(recordAt(currentSourceCredentialOutputRemediation, "verification"), "focusedAndAdjacentTests")?.testsPassed),
      ciCredentialTestsPassed: asNumber(recordAt(recordAt(currentSourceCredentialOutputRemediation, "verification"), "ciFullSuiteObservation")?.credentialOutputTestsPassed),
      ciMcpCliTestsPassed: asNumber(recordAt(recordAt(currentSourceCredentialOutputRemediation, "verification"), "ciFullSuiteObservation")?.mcpTokenCliTestsPassed),
      buildStatus: asString(recordAt(recordAt(currentSourceCredentialOutputRemediation, "verification"), "productionBuild")?.status),
      cliProbeCount: Array.isArray(recordAt(currentSourceCredentialOutputRemediation, "verification")?.failClosedCliProbes)
        ? recordAt(currentSourceCredentialOutputRemediation, "verification").failClosedCliProbes.length
        : 0,
      credentialIssuanceExecuted: recordAt(recordAt(currentSourceCredentialOutputRemediation, "verification"), "liveDeployment")?.credentialIssuanceExecuted === true,
      freshRescanRequired: recordAt(currentSourceCredentialOutputRemediation, "finding")?.freshRescanRequired === true,
      securityComplete: recordAt(currentSourceCredentialOutputRemediation, "remainingBoundaries")?.securityComplete === true,
      exactSavedShareVerdict: asString(recordAt(currentSourceCredentialOutputRemediation, "remainingBoundaries")?.exactSavedShareVerdict),
    },
    currentSourcePhotoReadinessAuthFanoutRemediation: {
      artifact: ARTIFACTS.currentSourcePhotoReadinessAuthFanoutRemediation,
      verdict: isRecord(currentSourcePhotoReadinessAuthFanoutRemediation)
        ? asString(currentSourcePhotoReadinessAuthFanoutRemediation.verdict)
        : "missing",
      sourceHead: isRecord(currentSourcePhotoReadinessAuthFanoutRemediation)
        ? asString(currentSourcePhotoReadinessAuthFanoutRemediation.sourceHead)
        : "",
      productionCommit: isRecord(currentSourcePhotoReadinessAuthFanoutRemediation)
        ? asString(currentSourcePhotoReadinessAuthFanoutRemediation.productionCommit)
        : "",
      findingId: asString(recordAt(currentSourcePhotoReadinessAuthFanoutRemediation, "finding")?.findingId),
      testsPassed: asNumber(recordAt(recordAt(currentSourcePhotoReadinessAuthFanoutRemediation, "verification"), "focusedTests")?.testsPassed),
      buildStatus: asString(recordAt(recordAt(currentSourcePhotoReadinessAuthFanoutRemediation, "verification"), "productionBuild")?.status),
      publicGetCreatesSupabaseAdminClient: recordAt(currentSourcePhotoReadinessAuthFanoutRemediation, "remediation")?.publicGetCreatesSupabaseAdminClient === true,
      publicGetCallsSupabaseAuthentication: recordAt(currentSourcePhotoReadinessAuthFanoutRemediation, "remediation")?.publicGetCallsSupabaseAuthentication === true,
      responseBodiesEqual: recordAt(recordAt(recordAt(currentSourcePhotoReadinessAuthFanoutRemediation, "verification"), "liveDeployment"), "publicProbe")?.responseBodiesEqual === true,
    ...
    hermesCandidateBodyHazardCoverage: {
      artifact: ARTIFACTS.hermesCandidateBodyHazardCoverage,
      verdict: isRecord(hermesCandidateBodyHazardCoverage) ? asString(hermesCandidateBodyHazardCoverage.verdict) : "missing",
      productCommit: isRecord(hermesCandidateBodyHazardCoverage) ? asString(hermesCandidateBodyHazardCoverage.productCommit) : "",
      canonicalHazardCount: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "contract")?.canonicalHazardCount),
      allMatchedHazardsMustAppearInCandidateBody: recordAt(hermesCandidateBodyHazardCoverage, "contract")?.allMatchedHazardsMustAppearInCandidateBody === true,
      metadataTraceAloneCanSatisfyBodyCoverage: recordAt(hermesCandidateBodyHazardCoverage, "contract")?.metadataTraceAloneCanSatisfyBodyCoverage === true,
      unknownHazardIdsFailClosed: recordAt(hermesCandidateBodyHazardCoverage, "contract")?.unknownHazardIdsFailClosed === true,
      approvalRequiresCanonicalTraceAndBodyCoverage: recordAt(hermesCandidateBodyHazardCoverage, "contract")?.approvalRequiresCanonicalTraceAndBodyCoverage === true,
      localVerdict: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.verdict),
      localCaseCount: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.caseCount),
      localPassed: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.passedCount),
      localFailed: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.failedCount),
      matchedHazardCount: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.matchedHazardCount),
      bodyGroundedHazardCount: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.bodyGroundedHazardCount),
      bodyHazardCoverageCompleteCount: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal")?.bodyHazardCoverageCompleteCount),
      multiHazardCaseId: asString(recordAt(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal"), "multiHazardCase")?.id),
      multiHazardMatched: asNumber(recordAt(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal"), "multiHazardCase")?.matchedHazardCount),
      multiHazardGrounded: asNumber(recordAt(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal"), "multiHazardCase")?.bodyGroundedHazardCount),
      multiHazardPassed: recordAt(recordAt(hermesCandidateBodyHazardCoverage, "afterLocal"), "multiHazardCase")?.passed === true,
      liveVerdict: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.verdict),
      sourceHead: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.sourceHead),
      productionCommit: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.productionCommit),
      sourceProductionAligned: recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.sourceProductionAligned === true,
      liveCaseCount: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.caseCount),
      livePassed: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.passedCount),
      liveFailed: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.failedCount),
      liveHttpStatus: asNumber(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.httpStatus),
      liveBlockerCode: asString(recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.blockerCode),
      generationReached: recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.generationReached === true,
      providerCallPerformed: recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.providerCallPerformed === true,
      dbMutationPerformed: recordAt(hermesCandidateBodyHazardCoverage, "afterLive")?.dbMutationPerformed === true,
      liveRuntimeValidationBlocked: isRecord(hermesCandidateBodyHazardCoverage) && hermesCandidateBodyHazardCoverage.liveRuntimeValidationBlocked === true,
      humanReviewCompleted: isRecord(hermesCandidateBodyHazardCoverage) && hermesCandidateBodyHazardCoverage.humanReviewCompleted === true,
      exactSavedShareVerdict: asString(recordAt(hermesCandidateBodyHazardCoverage, "remainingBoundaries")?.exactSavedShareVerdict),
      llmWikiPublication: asString(recordAt(hermesCandidateBodyHazardCoverage, "remainingBoundaries")?.llmWikiPublication),
      supabaseRlsLaunchIsolation: asString(recordAt(hermesCandidateBodyHazardCoverage, "remainingBoundaries")?.supabaseRlsLaunchIsolation),
      distributedAdmissionActivation: asString(recordAt(hermesCandidateBodyHazardCoverage, "remainingBoundaries")?.distributedAdmissionActivation),
    },
...
export function renderNorthstarLiveRollupMarkdown(rollup) {
  const lines = [
    ...
    "",
    `- Verdict: \`${rollup.hermesCandidateBodyHazardCoverage.verdict}\`; local cases ${rollup.hermesCandidateBodyHazardCoverage.localPassed ?? "unknown"}/${rollup.hermesCandidateBodyHazardCoverage.localCaseCount ?? "unknown"}, matched/body-grounded hazards ${rollup.hermesCandidateBodyHazardCoverage.matchedHazardCount ?? "unknown"}/${rollup.hermesCandidateBodyHazardCoverage.bodyGroundedHazardCount ?? "unknown"}, complete cases ${rollup.hermesCandidateBodyHazardCoverage.bodyHazardCoverageCompleteCount ?? "unknown"}/5, multi-hazard ${rollup.hermesCandidateBodyHazardCoverage.multiHazardMatched ?? "unknown"}/${rollup.hermesCandidateBodyHazardCoverage.multiHazardGrounded ?? "unknown"}.`,
    `- Live runtime remains a notice, not live quality proof: ${rollup.hermesCandidateBodyHazardCoverage.livePassed ?? "unknown"}/${rollup.hermesCandidateBodyHazardCoverage.liveCaseCount ?? "unknown"}, HTTP ${rollup.hermesCandidateBodyHazardCoverage.liveHttpStatus ?? "unknown"}, blocker \`${rollup.hermesCandidateBodyHazardCoverage.liveBlockerCode || "missing"}\`, generation/provider/DB reached=${rollup.hermesCandidateBodyHazardCoverage.generationReached === true}/${rollup.hermesCandidateBodyHazardCoverage.providerCallPerformed === true}/${rollup.hermesCandidateBodyHazardCoverage.dbMutationPerformed === true}.`,
    `- Boundaries: human review complete=${rollup.hermesCandidateBodyHazardCoverage.humanReviewCompleted === true}; exact saved Share=${rollup.hermesCandidateBodyHazardCoverage.exactSavedShareVerdict || "MISSING_EVIDENCE"}; Wiki/RLS/distributed admission=${rollup.hermesCandidateBodyHazardCoverage.llmWikiPublication || "APPROVAL_GATED"}/${rollup.hermesCandidateBodyHazardCoverage.supabaseRlsLaunchIsolation || "APPROVAL_GATED"}/${rollup.hermesCandidateBodyHazardCoverage.distributedAdmissionActivation || "APPROVAL_GATED"}.`,
    "",
