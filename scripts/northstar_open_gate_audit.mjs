...
const EVIDENCE_PATHS = Object.freeze({
  ...
  shareRecipientDisclosureAffordance: path.join("evaluation", "share-recipient-disclosure-affordance-2026-08-31", "after-live", "report.json"),
...
function evaluateUiDocumentsShareCockpitGate(rootDir) {
  ...
  const shareRecipientDisclosureAffordancePath = EVIDENCE_PATHS.shareRecipientDisclosureAffordance;
  ...
  const shareRecipientDisclosureAffordance = readJsonFile(rootDir, shareRecipientDisclosureAffordancePath);
  ...
  if (!isRecord(shareRecipientDisclosureAffordance)) {
    return gateResult({
      id: "ui_documents_share_cockpit",
      label: "Documents and Share cockpit UI",
      state: "missing",
      evidencePath: shareRecipientDisclosureAffordancePath,
      detail: "The live invited-recipient document disclosure affordance evidence is missing or invalid.",
      nextActions: ["Regenerate the live desktop/mobile invited-recipient disclosure evidence without claiming exact saved Share."],
    });
  }
  ...
  const shareRecipientDisclosureBuild = isRecord(shareRecipientDisclosureAffordance.productionBuild)
    ? shareRecipientDisclosureAffordance.productionBuild
    : {};
  const shareRecipientDisclosureResults = Array.isArray(shareRecipientDisclosureAffordance.results)
    ? shareRecipientDisclosureAffordance.results.filter(isRecord)
    : [];
  const shareRecipientDisclosureRows = shareRecipientDisclosureResults.filter((row) => (
    readString(row.route) === "/share/[sessionId] invited recipient fixture"
  ));
  const shareRecipientDisclosureAffordancePass = readString(shareRecipientDisclosureAffordance.verdict) === "PASS_LIVE_PRODUCTION_SCOPED_WORKSPACE_AND_INVITED_FIXTURE"
    && readString(shareRecipientDisclosureAffordance.mode) === "live-production"
    && readString(shareRecipientDisclosureAffordance.sourceHead) === readString(shareRecipientDisclosureBuild.commitSha)
    && readString(shareRecipientDisclosureBuild.branch) === "master"
    && readString(shareRecipientDisclosureBuild.environment) === "production"
    && readBoolean(shareRecipientDisclosureAffordance.providerDispatchLiveClaimed) === false
    && readBoolean(shareRecipientDisclosureAffordance.dbMutationPerformed) === false
    && readBoolean(shareRecipientDisclosureAffordance.exactSavedUserSessionReproduced) === false
    && readString(shareRecipientDisclosureAffordance.exactSavedSessionVerdict) === "MISSING_EVIDENCE"
    && shareRecipientDisclosureRows.length === 4
    && shareRecipientDisclosureRows.every((row) => (
      readString(row.verdict) === "PASS"
      && readBoolean(row.metrics?.documentsPanelOpen) === false
      && readString(row.metrics?.documentsSummaryAffordance) === "+"
      && readBoolean(row.metrics?.horizontalOverflow) === false
      && readNumber(row.metrics?.outsideElements) === 0
    ));
...
function evaluateFreshCurrentSourceSecurityScanGate(rootDir) {
  ...
  const pass = readString(report.verdict) === "NOTICE_CURRENT_SOURCE_STANDARD_SCAN_14_FINDINGS_PARTIAL_COVERAGE"
    && readString(report.scanId) === "a4044172-9b4b-4e36-84be-a8955d9150ac"
    && readString(report.sourceHead) === "121c8a017c18b58874ef965cece12bc3e0f0df2f"
    && readString(report.deployedProductSource) === "121c8a017c18b58874ef965cece12bc3e0f0df2f"
    && readString(scan.status) === "completed"
    && readString(scan.mode) === "standard"
    && readString(scan.targetKind) === "git_revision"
    && readString(scan.coverageCompleteness) === "partial"
    && readNumber(scan.trackedFileCount) === 6981
    && readNumber(scan.reviewWorklistCount) === 5
    && readNumber(scan.closedReviewWorklistCount) === 5
    && readNumber(scan.reviewedSurfaceCount) === 21
    && readNumber(scan.deferredCoverageItemCount) === 22
    && readNumber(scan.reportableFindingCount) === 14
    && readNumber(scan.uniqueFindingWriteupCount) === 14
    && readNumber(severity.critical) === 0
    && readNumber(severity.high) === 0
    && readNumber(severity.medium) === 10
    ...
    && readNumber(disposition.approvalFreeProductSourceResidualCount) === 7
    && readNumber(disposition.fullyClosedBoundedSourceCandidateCount) === 0
    && disposition.securityCompleteClaimAllowed === false
    && readNumber(canonical.findingWriteupCount) === 14
    && readNumber(canonical.supportingEvidenceCount) === 14
