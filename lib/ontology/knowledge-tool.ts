...
export function buildPublishedSafetyKnowledge(
  ...
  const failClosed =
    !evidenceResolution.resolved && evidenceResolution.reason !== "not_registered";
  const coreQuery = evidenceResolution.resolved ? evidenceResolution.pack.task.label : query;
