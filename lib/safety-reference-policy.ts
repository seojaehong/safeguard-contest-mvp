...
export type SafetyReferenceRetrievalMode = "unconfigured" | "rest-ilike" | "ranked-rpc" | "hybrid-vector-rpc" | "hybrid-local-supabase" | "local-tag" | "local-ranked" | "local-hybrid";
...
export function normalizeReferenceItem(item: SafetyReferenceItem): SafetyReferenceItem {
...
export function extractFallbackTerms(value: string): string[] {
...
export function parseContentRange(value: string | null): number {
  if (!value) return 0;
...
export function withRetrievalSource(
...
export function mergeLocalAndRemoteSafetyReferenceResults(input: {
  ...
  const remoteItems = Array.from(remoteById.values());
