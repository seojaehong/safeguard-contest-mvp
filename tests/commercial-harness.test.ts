...
describe("runAsk DB harness mode", () => {
  ...
  it("preserves explicit multi-process labels in deterministic DB harness risk rows", () => {
    ...
    const references = [
      reference({
        id: "sif-excavation-collapse",
        title: "굴착 작업 절토사면 낙석 사고 사례",
        summary: "굴착 작업 중 절토사면 하부에서 낙석에 맞은 사례",
        keywords: ["굴착", "절토사면", "낙석"],
        risk_tags: ["낙하", "붕괴"],
        controls: ["굴착면 상태와 낙석 위험 확인", "굴착 하부 출입통제"],
        evidence_role: "direct",
        retrieval_source: "rest"
      }),
      reference({
    ...
    const processes = rows.map((row) => row.process);

    expect(processes.filter((process) => process === "굴착").length).toBeGreaterThanOrEqual(2);
    expect(processes.filter((process) => process === "토사반출").length).toBeGreaterThanOrEqual(2);
    expect(processes.filter((process) => process === "자재양중").length).toBeGreaterThanOrEqual(2);
    expect(rows).toHaveLength(6);
    expect(validateRiskAssessmentRows(rows).issues).toEqual([]);
  });

  it("keeps deterministic multi-process rows tied to process-specific evidence instead of borrowing adjacent process refs", () => {
    const question = "서울 성수동 복합건물 현장. 굴착, 토사반출, 자재양중 3개 공종을 오늘 동시에 진행한다.";
    const response = buildMockAskResponse(
      "굴착, 토사반출, 자재양중 3개 공종 동시 작업",
      mockSearchResults,
    ...
    const rows = buildSafetyReferenceRiskRows(response, references, "맑음", question);
    const processTerms: Record<string, RegExp> = {
      굴착: /굴착|절토|낙석|사면/,
      토사반출: /토사반출|토사 반출|덤프트럭|후진|가설도로/,
      자재양중: /자재양중|자재 양중|인양|줄걸이|크레인/
    };

    for (const [process, pattern] of Object.entries(processTerms)) {
      const processRows = rows.filter((row) => row.process === process);
      ...
      for (const row of processRows) {
        ...
        expect(rowText).toMatch(pattern);
      }
    }
  });

  it("does not use another explicit process accident case just because its summary mentions the current process", () => {
    const question = "서울 성수동 복합건물 현장. 굴착, 토사반출, 자재양중 3개 공종을 오늘 동시에 진행한다.";
    const response = buildMockAskResponse(
      "굴착, 토사반출, 자재양중 3개 공종 동시 작업",
      mockSearchResults,
    ...
    const references = [
      reference({
        id: "sif-excavation-collapse",
        title: "굴착 절토사면 낙석 사고 사례",
        summary: "굴착면 하부에서 작업자가 낙석에 맞은 사례",
        keywords: ["굴착", "절토사면", "낙석"],
        risk_tags: ["낙하", "붕괴"],
        controls: ["굴착면 상태와 낙석 위험 확인", "굴착 하부 출입통제"],
        evidence_role: "direct",
      ...
      reference({
        id: "sif-soil-haulage-mentions-excavation",
        title: "토사반출 덤프트럭 충돌 사고 사례",
        summary: "토사반출 차량 진출입로 주변 굴착작업이 있던 현장에서 덤프트럭이 보행자를 충돌한 사례",
        keywords: ["토사반출", "덤프트럭", "충돌"],
        risk_tags: ["충돌"],
        controls: ["토사반출 차량 동선과 보행 동선 분리", "후진 구간 신호수 배치"],
        evidence_role: "direct",
    ...
    const rows = buildSafetyReferenceRiskRows(response, references, "맑음", question);
    const excavationRows = rows.filter((row) => row.process === "굴착");
    const excavationText = excavationRows.map((row) => [
      row.hazard,
      row.currentControls,
      row.additionalControls,
      ...row.evidenceRefs
    ...
    expect(excavationRows).toHaveLength(2);
    expect(excavationText).toContain("굴착 절토사면 낙석 사고 사례");
