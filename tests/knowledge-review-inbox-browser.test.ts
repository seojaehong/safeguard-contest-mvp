...
describe("knowledge review inbox browser", () => {
  ...
  it("shows one selected mobile-safe candidate and submits the existing approval action", async () => {
    ...
    const structuredCandidate = inbox.locator('[data-review-candidate-format="structured"]');
    expect(await structuredCandidate.count()).toBe(1);
    ...
    expect(await inbox.locator('[data-review-candidate-format="raw"]').count()).toBe(1);
    expect(await inbox.locator('[data-review-candidate-section]').count()).toBe(0);
