...
type RollupReport = {
  ...
  currentSourceKoshaArchivePreflightRemediation: {
    ...
    buildStatus: string;
    boundedPythonInventoryUsed: boolean;
...
function freshCurrentSourceSecurityScanFixture(): Record<string, unknown> {
  return {
    ...
    postScanRemediation: { browserSessionBindingProductCommit: "e36356d84fa8ac2331f8d0b81229d0532024a876", browserSessionBindingVerdict: "PASS_LIVE_DEPLOYED_SOURCE_BROWSER_SESSION_BINDING_RESCAN_PENDING", browserSessionBindingProductionCommit: "77bcd6ea4ec1ea1914126dd7ba924f788b972602", providerOutputBudgetProductCommit: "e99346797f4d874e83188d8f37941a603a272a6d", providerOutputBudgetEvidenceCommit: "18a9fc1a", providerOutputBudgetVerdict: "PASS_LIVE_DEPLOYED_SOURCE_PUBLIC_PROVIDER_OUTPUT_BUDGET_RESCAN_PENDING", providerOutputBudgetProductionCommit: "18a9fc1a7f9a549d73474d5fb881268bb69404d9", scanFindingReclassified: false, remainingApprovalFreeFindingCount: 5 },
...
function createFixtureRoot(): { root: string; head: string } {
  ...
  writeJson(root, "evaluation/northstar-open-gates-current/report.json", {
    ...
    gates: [
      ...
      { id: "fresh_current_source_security_scan", state: "notice", evidencePath: "evaluation/current-source-standard-security-scan-2026-08-31-121c8a01-complete/report.json", detail: "14 findings with partial coverage; six approval-free current-source residuals and seven approval-gated database findings remain open" },
  ...
  writeJson(root, "evaluation/hermes-knowledge-review-structured-sections-2026-08-28/report.json", {
  ...
  [
    ...
    "evaluation/hermes-knowledge-review-structured-sections-2026-08-28/report.json",
...
describe("northstar live rollup", () => {
  it("binds mobile fixed and open-gate evidence to the current production build", () => {
    ...
    expect(report.evidence.find((item) => item.id === "fresh_current_source_security_scan")).toMatchObject({
      artifact: path.join("evaluation", "current-source-standard-security-scan-2026-08-31-121c8a01-complete", "report.json"),
    });
    expect(report.freshCurrentSourceSecurityScan).toMatchObject({
      verdict: "NOTICE_CURRENT_SOURCE_STANDARD_SCAN_14_FINDINGS_PARTIAL_COVERAGE",
      scanId: "a4044172-9b4b-4e36-84be-a8955d9150ac",
      sourceHead: expect.stringMatching(/^[0-9a-f]{40}$/u),
      deployedProductSource: expect.stringMatching(/^[0-9a-f]{40}$/u),
      reportableFindingCount: 14,
    ...
    expect(report.evidence.find((item) => item.id === "current_source_photo_readiness_auth_fanout_remediation")).toMatchObject({
      artifact: path.join("evaluation", "current-source-security-photo-readiness-auth-fanout-remediation-2026-08-31", "report.json"),
    });
    ...
    expect(report.evidence.find((item) => item.id === "current_source_kosha_archive_preflight_remediation")).toMatchObject({
      artifact: path.join("evaluation", "current-source-security-kosha-archive-preflight-remediation-2026-08-31", "report.json"),
    });
