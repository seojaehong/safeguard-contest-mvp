# SafeClaw Document Focus Mode Implementation Plan

## Global Constraints

- Preserve the existing `input -> document -> share` product direction.
- Do not change Supabase schema or data.
- Do not change DB harness, ontology, evidence, or share-readiness semantics.
- Use TypeScript strict mode without `any`.
- Follow TDD and keep the existing RED evidence in `tests/workspace-layout-regression.test.ts`.
- Use `npm.cmd`, never `npm`.
- Do not touch unrelated untracked evaluation or output artifacts.
